...
for file in "$ROOT/.env.example" "$ROOT/deploy/install.sh"; do
	grep -F "$KEY" "$file" >/dev/null || {
		echo "trusted release public key missing from $file" >&2
		exit 1
	}
done
grep -F 'TUNNEX_RELEASE_KEY_ID=release-2026-08-01' "$ROOT/.env.example" "$ROOT/deploy/install.sh" >/dev/null || {
