...
| **F14 — MCP per-tool policy enforcement** | Add versioned allow/deny rules keyed to stable MCP server and tool identities. Newly discovered tools remain denied until explicitly allowed. | Validate authoritative MCP request content, reject header/body mismatches, and enforce tool policy before forwarding. Keep L3/L4 enforcement as the fail-closed network boundary. | Add server-to-tool permission selection with a clear default-deny state. | 🟨 Local development complete; live box-walk pending |
