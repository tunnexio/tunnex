...
**ACTIVE LOCAL (2026-09-07): AI gateway AI-0–AI-5 implementation and local qualification on `ai-improvement`, content tip `b1836352f3f159d684d74c25d190b8e831eb1a6b`, API/UI content `c1a27c9b`, rebased onto verified main `26a36afc`; NOT PUSHED OR MERGED. Community default-OFF, one explicit AI team with narrowing overrides and honest soft admission thresholds are implemented. Full API suites/builds in both editions, pinned native gateway/HTTP race suites in both editions, 1,314 web tests/typecheck/build, CLI tests/build, 50-file zero codegen drift and affected vet pass. Actual isolated Linux API-process walkthrough passed real admin login, canonical agent enrollment, team isolation, complete SSE, priced threshold refusal, revocation/restart and content omission; independent provider/admin rotation and same-version restore also passed. Results and limits: `docs/AI-2-5-validation-20260907.md`, `docs/AI-5-installed-process-walk-20260907.md`. Public guide is local `tunnex-web` branch `ai-improvement` at `621b8d7` in `/private/tmp/tunnex-web-ai-improvement`; original dirty website checkout/stash preserved. Required exact-head remote CI is still unrun; packaged image/public TLS/Helm-CNI/cross-version/HA claims remain excluded. All walkthrough containers stopped, evidence volumes retained; no cloud operations. NEXT ACTION: obtain push/PR authorization for these committed candidates and run current required GitHub checks on the exact head. No merge/release authorization. Earlier checkpoints below are historical.**
