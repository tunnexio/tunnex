...
**LOCAL RUNTIME EVIDENCE (2026-09-24, supersedes below): Pure node evidence evaluator implemented without production wiring. Exact assignment/revision/ownership and route sets, fresh consistent inventory, scoped refusal, established SA and policy checks yield candidate evidence only. All17 test groups (7 independent adversarial) pass with race detection; five behavioral guard mutants killed. Linux test compilation/node build and capability0 regression pass; independent review clear. No daemon, kernel, dependency, schema, UI, CP mutation or activation. Existing local UI remains available. No commit/push. Next runtime integration still requires the engine/allocator/durable-delivery/cleanup decisions in the runtime proposal, followed by actual adapter and packet-path qualification. [Evidence contract](docs/S-S2S-2-runtime-evidence.md).**

**LOCAL MANAGEMENT UI (2026-09-24, supersedes below): IPsec method workspace, explicit opt-in, paginated inventory, two-step disabled creation, nonsecret readback and exact-revision delete implemented. Read-only eligibility shares authoritative create gates. Scope-loading clears drafts immediately; uncertain retries reconcile the same identity before readiness checks. Full web1604 tests/135 files and production build pass; API race/DB eligibility tests, deterministic generation and both edition builds pass. Independent review clear. Desktop main-app opt-out state and synthetic form/readback inspected. Owned local CP refreshed, health/proxy200 and anonymous eligibility401. Schema159 unchanged; production nodes capability0. No commit/push. Runtime activation pending. [UI contract and evidence](docs/S-S2S-2-management-ui.md).**

**LOCAL PROVIDER API (2026-09-24, supersedes below): public HTTP POST connections now wraps qualified disabled provider creation, with verified-human authorization before secret-body parsing and unchanged opt-in/ownership/fresh capability gates. GET connection/configuration uses org:view and an explicit credential-free projection, no-store and tombstone metadata. Caller UUID retries conflict without replacing PSKs. Real-router/PostgreSQL lifecycle, security/race, two overlay mutations, existing HTTP/resource guards, auth census, both builds, deterministic generation, CLI compile and web typecheck pass; independent review clear. Owned local CP API refreshed at unchanged schema159, health/UI proxy pass and both new routes reject anonymous requests. UI/theme unchanged; production nodes remain capability0. No runtime activation or live VPN claim, no commit/push. Next: concise creation/readback UI respecting these prerequisites, followed by separately disposed runtime qualification. [API contract and evidence](docs/S-S2S-2-provider-api.md).**

**LOCAL PROVIDER STORAGE (2026-09-24, supersedes below): after the user instructed the next slice, migration159 and internal create/read/delete provider storage implement the reviewed create-only AWS static IPv4 ownership contract. Exact approved-local-subnet references, org-exclusive remote reservations, gateway-scoped tunnel uniqueness, underlay protection, immutable sealed configuration and retained tombstones now have mirrored database/shared-writer guards. Raw Site-org movement and stale-snapshot races are covered. Regression/race, six guard mutations, legacy compatibility, HTTP/resource guards, both API builds, deterministic SQLC and independent reviews pass. Owned local CP is schema159 clean, API refreshed and local HTTP smoke passes. UI/theme unchanged; no public provider-create, runtime activation or live VPN claim. HEAD remains `0b90d814`, all changes local/uncommitted, no push. Next: separate public creation/configuration UX contract and runtime qualification; current agents remain capability0. [Implementation and evidence](docs/S-S2S-2-provider-persistence.md).**

**LOCAL CONFIGURATION-CHECK API (2026-09-24, supersedes below): authenticated nonpersisting POST configuration-check now exposes the bounded static IPv4 preflight with verified-human permission before credential body reads, bounded duplicate-aware JSON and static redacted errors. Route regressions/race, 317-operation auth census, two source-overlay security mutants, both API edition builds, generated CLI compile and web typecheck pass; pinned generated artifacts are deterministic and independent review is clear. Owned local CP refreshed and health/UI proxy smoke checks pass; live unauthenticated check returns401. Existing UI/theme unchanged. Provider persistence design is reviewed but awaits explicit disposition of new shared ownership/range guards; no migration159, provider create, activation or cloud work. Local/uncommitted HEAD remains `0b90d814`; no push. [API evidence](docs/S-S2S-2-config-preflight.md), [pending provider persistence design](docs/S-S2S-2-provider-persistence.md).**

**LOCAL MULTI-AGENT PREFLIGHT (2026-09-24, supersedes below): pure AWS static-IPv4 preflight implemented with bounded canonical routed prefixes, explicit customer/cloud outside addresses, two inside /30 assignments, overlap/special-use rules and write-only AWS PSKs. Regression/race, four guard mutations, API build and independent review pass. Parallel primary-source provider research and Linux strongSwan/VICI/XFRM runtime plan are documented. Helper remains unwired: no persisted provider config/public creation/runtime activation added, no DB/schema/cloud/install action. Current nodes retain capability0. Local/uncommitted HEAD `0b90d814`, no push. Next: concretize persisted provider configuration plus unresolved runtime ownership/packaging decisions before live implementation. [Validation evidence](docs/S-S2S-2-config-preflight.md), [provider profile](docs/S-S2S-2-provider-profile.md), [runtime proposal](docs/S-S2S-2-runtime-plan.md).**

**LOCAL CAPABILITY/CREATE CONTINUATION (2026-09-24, supersedes below): private authenticated report now carries normalized `ipsec_config_version`, with existing server `policy_reported_at` as evidence. Current production client explicitly reports unsupported0. Internal-only `CreateDisabled` requires opt-in, scoped active enrolled gateway, exact version1 and a nonfuture receipt within90s, then atomically stores two bound PSKs and redacted audit. No public create route/provider fields/runtime activation. Isolated DB rollback/race tests, four observed-lock competitor tests, five behavioral mutations, all IPsec HTTP tests, node control race tests and both API builds pass. Synthetic positive capability only; no review-CP records seeded or schema changed. Local/uncommitted HEAD remains `0b90d814`; no push. Next: provider configuration validation and runtime contract/engine qualification before publicly accepting usable configurations. [Contract and evidence](docs/S-S2S-2-persistence-contract.md).**

**LOCAL API CONTINUATION (2026-09-24, supersedes below): public IPsec connection list/detail/delete now wrap the qualified dormant store. UUID keyset pages are bounded1–100, detail/delete return strong revision ETags, DELETE requires exact canonical If-Match plus verified human `ipsec:manage`; all responses are explicitly nonsecret. Real-router/PostgreSQL integration, race gates, authorization census, five page/permission/header mutants, both API builds, CLI generated compile and web typecheck pass. Schema158 unchanged, no live IPsec records seeded. Existing UI/theme retained. Still local/uncommitted at HEAD `0b90d814`; no push. Next dependency: decide and implement gateway capability/creation-validation contract before accepting configurations; no create/activate/runtime claim. [Contract/evidence](docs/S-S2S-2-persistence-contract.md).**

**LOCAL CONTINUATION (2026-09-24, supersedes below): internal redacted IPsec connection Read/List and exact-revision never-delivered Delete implemented, with atomic secret withdrawal, retained tombstone/history and redacted audit. Existing site/node destructive services map only the two IPsec resource FK constraints to scoped 409. Isolated PostgreSQL regressions and race gate pass; four guard mutations killed/restored; API build and independent review clear. No new public connection routes or schema changes. Gateway IPsec capability wire/engine semantics remain undefined: creation and activation stay unavailable. Next: specify capability/creation validation and public connection API contracts before accepting configurations; org-delete error mapping and public pagination also remain. Local-only instruction stays in force; HEAD `0b90d814` unchanged. [Evidence and remaining boundaries](docs/S-S2S-2-persistence-contract.md).**

**LOCAL SITE-TO-SITE CHECKPOINT (2026-09-24, supersedes below): founder requested local-only development; no further push until later review/testing. HEAD remains `0b90d814`, with the organization opt-in settings service/API and compact UX uncommitted. GET is pure default-off; verified human `ipsec:manage` PUT uses exact revision CAS and atomic redacted audit. Isolated PostgreSQL tests, race gate, real router/store integration, authorization census, four behavioral guard mutations and API build pass. UI now has one Site-to-site sidebar entry, Networks/Connectivity tabs, concise pair selection with Swap/Reset and a collapsed setup guide, using the existing theme. Typecheck, 1,587 web tests and production build pass; browser interactions inspected and independent review clear. Local CP remains schema 158; no activation/runtime or live traffic claim. NEXT: disabled connection service lifecycle within the approved foundation, with regression proof before writes; runtime qualification remains separate. [UX evidence](docs/S-S2S-1-decisions.md), [settings contract and evidence](docs/S-S2S-2-persistence-contract.md).**

**SITE-TO-SITE CHECKPOINT (2026-09-24, supersedes the earlier checkpoint below): migration 0158 now adds dormant disabled/deleted IPsec storage with scoped ownership, two-tunnel/secret integrity and tombstone preservation. PostgreSQL up/down/up, refusal/concurrency tests and race gate pass; a demonstrated Repeatable Read deletion gap was fixed before completion. Five guard mutants killed and exact source restored; SQLC generation deterministic; API builds; independent review clear within schema scope. Local real CP `tunnexs2scp0924` is running at schema 158 clean with authenticated UI at http://127.0.0.1:5198/site-to-site; isolated migration tests use separate tmpfs project `tunnexs2sfoundation0924`. No IPsec service/API/runtime or support claim yet. NEXT: internal service transactions with authority/capability/opt-in, CAS and redacted audits, then generated OpenAPI management routes; keep runtime/packet-path qualification separate. [Persistence contract and evidence](docs/S-S2S-2-persistence-contract.md).**

**ACTIVE SITE-TO-SITE (2026-09-24): `story/site-to-site-connectivity`, isolated checkout `/private/tmp/tunnex-site-to-site-dev`, baseline `1a81f8fc`. UI inventory/pair review, gateway diagnostics (`71738b22`) and served transit hubs (`5d9a9c03`) pushed; desktop synthetic preview remains `http://127.0.0.1:5198/.s2s-preview/index.html` and its fixture is untracked. Founder approved IPsec foundation and explicitly chose Community, Trial and paid tiers: no licence-tier restriction. First backend increment adds dedicated owner/admin `ipsec:manage` plus bound encrypted PSK primitives; race tests, real authorization-seam test, API build, web typecheck/build and all 1,583 web tests pass. Six envelope guard mutants killed and restored; independent reviews found no actionable findings. No IPsec endpoint/schema/delivery/runtime exists yet; UI remains unavailable. No PR/exact-head PR CI, merge/release/CP/cloud deployment. Original dirty checkout preserved. NEXT: persistence/API lifecycle design and regression proof, then isolated DB gates before any persistence claim; engine and packet-path qualification remain separate. [S2S-1 evidence](docs/S-S2S-1-decisions.md), [S2S-2 foundation](docs/S-S2S-2-decision-gates.md). Earlier checkpoints below are historical.**

**ACTIVE PR #72 (2026-09-13): Agent rotation recovery and MCP OAuth walk, content tip `d62209e7` (pre-merge), branch `codex/mcp-oauth-refresh-walk`. Based on merged PR #71 / main `1ed535ea`. Runtime preserves prepared credentials across interrupted handoffs; API retries use the next unused revision while preserving revoked history. Live crash/restart, predecessor 401, successor 200, and runtime/WireGuard convergence are recorded in `walk-artifacts/agent-rotation-recovery-20260912/README.md`; shortened-lease OAuth/cache proof is in `walk-artifacts/mcp-oauth-refresh-20260912/README.md`. API hotfix and disposable runtime deployed; scratch watcher/test DB cleaned. Required CI remains pending; no merge authorized. Root checkout unrelated edits preserved. NEXT: review PR #72 and required checks on its exact head, then obtain explicit merge sign-off. Earlier checkpoints below are historical.**

**ACTIVE PR (2026-09-09): AI Gateway, group access and workload identities — PR #67, content tip `907507637e6589dd82bd3b73c5399ae39819ea6d` (pre-merge), remote branch `story/S-AI-workload-identity`. Main NAT PR #66 (`219d422b`) integrated; main owns migrations139–141, unpublished AI now142–154 and156. Previous API/web/CLI/generation/migration and security/contract evidence is in `docs/S-AI-publication-validation.md`. Stale Users browser selectors corrected for Users & Groups and role sets; all seven local browser regressions and E2E typecheck pass. User authorized merge if CI is green; final core CI and GitHub code-owner approval remain required. Companion website PR #48 merged at `914b9c7a0cee742161234850041eaf0c9e231da2` after both checks passed. Deferred workload/W7/MCP/production qualifications remain documented. Original preview DB and unrelated work preserved; no production deployment. NEXT: assess final-head CI and required GitHub approval, then merge with the exact-head guard. Earlier checkpoints below are historical.**

**PREVIOUS LOCAL (2026-09-09): AI console navigation/design, invitation roles and threshold fixture complete on `ai-improvement`, content tip `266d1372024cb0d5710c165bc8465602a7561d3c`; NOT PUSHED OR MERGED. AI Gateway/Agents/MCP have separate sidebar homes; people groups live under Users & Groups. All human invitation roles are supported end to end, resends preserve the role, owner-only grants remain protected, and the role-count card is removed. Isolated preview schema151; named unassigned AI threshold UI fixture retains USD2.50 at revision4. Web1473, website214, both API builds, full enterprise DB suite, open DB suite with invitation rerun after test-schema correction, CLI and Linux node tests pass. Exact synthetic native-cost refusal/restart checks and zero-drift generation pass. Browser review covers top-level AI pages, forms and agent-detail empty/gated states. Website latest main pulled in clean worktree `/private/tmp/tunnex-web-ai-improvement`; docs commit `e23eed8e0158351fbee86730e707592348b10d2d`. Existing unrelated work preserved. Evidence `docs/S-AI-navigation-design-followup-boxwalk.md`. NEXT: user reviews local UI/docs; publication/merge, remote CI, handset acceptance and actual private-Azure reachability remain separate. Earlier checkpoints below are historical.**

**PREVIOUS LOCAL (2026-09-08): Test Connect diagnostics on `ai-improvement`, content tip `af25964e4e4e5a5ce264cc81d36e6e69b6af29cc`; NOT PUSHED OR MERGED. Paper `e48fb066`; approved outer-timeout, incomplete-response and gateway-guidance folds included. Draft and saved-key tests preserve actual provider/proxy HTTP status and use bounded no-code network/timeout failures. Local API/engine/bridge updated. Browser proved draft provider401, proxy403, saved new-model chat success and saved provider404; no models/credentials saved, five models/four credentials and schema150 unchanged. Full web1460, focused76, SDK145, enterprise API and CLI pass; open API passed with two timing-test packages passing on serial rerun; codegen50 files has zero second-pass drift. Only the approved Docker VM restarted; Mac not restarted. Twelve containers restored and task compiler scratch removed. Manual preview relay/fixture mapping restoration remains required after future restarts. Evidence `docs/S-AI-test-diagnostics-boxwalk.md`. NEXT: user refreshes the local UI and runs Test Connect against their private Foundry deployment; actual Azure reachability, user visual acceptance, full composite gates and remote CI are unproven. Earlier checkpoints below are historical.**

**PREVIOUS LOCAL (2026-09-08): Saved-credential onboarding and new-model testing on `ai-improvement`, content tip `1052bafd2ede4519db78f2b6355221d79f42afbf`; NOT PUSHED OR MERGED. Papers `7063d346` and `7b4aa384`. Saved credentials are offered before provider selection; selection fills provider and hides/clears new key and endpoint fields. Genuine HTTP 200/success emits Sonner and enables Add Model; stale/error results refuse success. A private extension of pinned Bifrost v2.0.0 resolves saved keys inside the engine and tests existing/new models through the existing LiteLLM bridge without changing serving scope. OpenAPI generated; both API editions tested/built, enterprise focused race checks, 68 focused/1436 full web tests, TypeScript/build, 106 SDK tests and three final native Foundry wire cases pass. Local labelled `tunnexaiwalk0907repro4` engine/API refreshed. Browser proved saved `private-demo` and new `private-demo-new` HTTP 200, toast and enabled Add Model using synthetic credentials; no model saved. All provider rows and schema148 unchanged. Approved Docker cleanup removed 28 stopped containers and unused images/cache; isolated VM disk expanded to16GB, macOS not restarted,12 services restored,4GB free at final check. A rollback engine container remains retained. Real Azure test/user visual sign-off, full composite gates/remote CI and broader LiteLLM parity remain outstanding. NEXT: user refreshes local UI and tests their Azure credential/deployment. Evidence `docs/AI-saved-credential-onboarding-validation-20260908.md`. Earlier checkpoints below are historical.**

**PREVIOUS LOCAL (2026-09-08): Foundry portal URL/catalog correction on `ai-improvement`, content tip `9d264cf59b3f9f06e9566c772abb24439e3d6898`; NOT PUSHED OR MERGED. Paper `cf691248`. Complete Azure portal deployment URLs now import to the existing v1 resource route with explicit request preview; deployment/mode conflicts explained, unsafe URL forms refused. Azure AI Foundry label no longer OpenAI-only; pinned LiteLLM snapshot expanded to351 azure/azure_ai rows, including Llama/DeepSeek/Phi/Mistral suggestions. Non-OpenAI SDK probe token handling corrected.100 final focused web checks,1431 full web checks before final extra refusal case, TypeScript/build,103 SDK cases, affected API tests in open/enterprise-race, both API builds and actual pinned native Foundry fixture pass. Local labelled `tunnexaiwalk0907repro4` CP/SDK refreshed; schema148 clean,3 credentials/4 models unchanged. Separate browser verified9 Llama suggestions and enabled Test Connect for the supplied URL structure; synthetic draft cleared afterward. No live Azure/key submission or user visual sign-off. Literal legacy-version requests, Anthropic Messages and legacy /models adapters remain outstanding; portal input uses v1 implicit versioning. No schema/codegen change. Full LiteLLM parity/engine migration, broader provider forms/model-less credentials and detail pages remain outstanding as described below; full composite gates/remote CI unproven. NEXT: user tests actual Azure deployment/key in refreshed UI. Evidence `docs/AI-foundry-onboarding-validation-20260908.md`. Earlier checkpoints below are historical.**

**PREVIOUS LOCAL (2026-09-08): Eight model modes, public endpoint entry and automatic LiteLLM catalog search on `ai-improvement`, content tip `6998f987a085f14c54180a7548e6e67dbdde2026`; NOT PUSHED OR MERGED. Latest UI correction (paper `dd2aaf75`): Test Connect retains selected/typed draft models across mode changes, preserves saved modes and invalidates old probe success; explicit validation reasons now explain unselected models, keys, endpoints and setup. Reproduced and visually verified in a separate local Azure form using synthetic input only; the user's exact original filled sequence was unavailable.61 focused/1393 full web tests, TypeScript/build and independent narrow review pass. No provider inference, saved data or runtime changes in this follow-up. Eight-mode backend content remains `786e09cc`. Mode paper `41a29d45` plus numerical video bounds `3ff524b9`; public/search slice `17f7e4c6`. All eight requested modes selectable and persisted per model: chat, completion, embedding, audio speech/transcription, image/video generation and rerank. Actual LiteLLM SDK preflight dispatch, exact scoped API routes and retained Bifrost adapters implemented. Migrations0147–0148 preserve old chat mappings and own video jobs with opaque handles, scoped idempotency,24h retention, lease/expiry/revocation checks and bounded admission. Both API editions compile and affected PostgreSQL/HTTP race tests pass; actual pinned native wire fixtures prove all eight modes plus video create/status/content and cross-scope refusal.97 SDK tests,59 focused/1391 full web tests, TypeScript/build and two-pass deterministic codegen pass. Independent review findings fixed and re-reviewed. Local CP/UI updated in verified `tunnexaiwalk0907repro4`, schema148 clean,3 credentials/4 models preserved, zero video submissions. Eight-option dropdown and automatic mode-filtered search visually inspected at `http://127.0.0.1:5180/agents/ai-gateway`. Public HTTPS endpoints no longer need per-destination approval; private/internal destinations retain explicit network rules. No live Azure/AWS/media provider test or user visual sign-off. Non-token modes refuse monetary policies until unit/terminal cost accounting is qualified; provider/model support still requires its actual test. Serving engine remains Bifrost, preflight is LiteLLM SDK. Full LiteLLM engine migration, model-less credentials, broader provider forms and model-detail parity remain outstanding. Full composite gates and exact-head remote CI remain unproven; no push/merge/release/cloud authorization inferred. NEXT ACTION: let the user test the chosen Azure deployment and supported mode using their real key entered directly in UI, then commit sanitized provider evidence. Evidence `docs/AI-mode-validation-20260908.md` and `docs/AI-self-service-validation-20260908.md`. Earlier checkpoints below are historical.**

**PREVIOUS LOCAL CHECKPOINT (2026-09-08): Azure AI Foundry (OpenAI v1) provider on `ai-improvement`, content tip `3190393e74094696839e1daccf6b39f8f90e9c43`; NOT PUSHED OR MERGED. Decision paper `a95f605e`. Actual Azure resource API base/key/deployment fields, Azure logo, exact endpoint-kind allowlist and connection-owned model scope implemented through existing OpenAI-compatible SDK/native routing. Migration0146; both API editions compile and focused tests pass, actual isolated PostgreSQL open/enterprise-race proof passes, 55 bridge tests and pinned Bifrost Foundry streaming/nonstreaming synthetic transport pass.32 focused/1364 full web tests, typecheck/build and two-pass deterministic generation pass. Local CP/UI updated in verified `tunnexaiwalk0907repro4`, schema146 clean, provider rows unchanged (3 credentials/4 models); actual endpoint/key fields visually inspected. No actual Azure resource URL supplied: Setup required and Test Connect disabled until configured, no live Azure inference proof or user visual approval. Chat completions only; legacy Foundry protocols, reasoning-model probe compatibility and other modes excluded. Full LiteLLM OSS proxy migration, model-less credentials, additional modes/provider adapters and model detail remain outstanding. Full composite gates and exact-head remote CI remain unproven. NEXT ACTION: obtain the actual Azure resource URL, configure its exact destination/IP ranges in existing CP/egress and SDK bridge policy, then let the user enter the deployment/key and run Test Connect. Evidence `docs/AI-foundry-validation-20260908.md`. Earlier checkpoints below are historical.**
**Update this on every merge (one line) — a stale pointer re-enters a fresh session in the wrong epic.**

**PREVIOUS LOCAL CHECKPOINT (2026-09-07): Actual LiteLLM OSS proxy preparation on `ai-improvement`, content tip `642af8a93dcfc1cd7e72a2c400a812a728f12a5d`; NOT PUSHED OR MERGED. User now requests the actual model-call proxy, superseding the earlier Bifrost-only integration recommendation. Paper `70957bb1`, endpoint UI `d03ec446`. Full pinned OSS LiteLLM1.100.0 server runtime, separate private Compose/DB and independent secret guards prepared in `apps/ai-proxy`; proprietary optional enterprise package excluded without altering license checks. Five guard tests, full server import, CLI/offline initialization and Compose validation pass; no listening-server or inference proof. Current CP still routes inference to Bifrost. Duplicate approved-endpoint dropdown removed; typed API base and server allowlist remain.27 focused/1359 full web tests, typecheck/build and rendered local UI checks pass. Runtime capacity verified in context `colima-tunnex-sso-review`, project `tunnexaiwalk0907repro4`: Docker data filesystem99 percent used,82MB available. No cleanup, restart, resize, image build, schema provisioning or engine cutover performed. Full gates and exact-head remote CI remain unproven. Independent model-less credentials, additional modes, broader provider registry and model details remain outstanding. NEXT ACTION: disposition the concrete per-org transition (credential re-entry, scoped internal model aliases and preserved old/new usage) in `docs/S-AI-litellm-proxy-decisions.md`, provide isolated local runtime capacity, then implement admission/ownership integration and prove real scoped model calls. Evidence `docs/AI-litellm-proxy-preparation-20260907.md`. Earlier checkpoints below are historical.**

**PREVIOUS LOCAL CHECKPOINT (2026-09-07): AI gateway AI-0–AI-5, provider UI and LiteLLM SDK integration on `ai-improvement`, content tip `6d3882d6900cb19f09c2f0a6e7beb789a76db1a9` (implementation `3fdddc20`), baseline main `26a36afc`; NOT PUSHED OR MERGED. Community default-OFF. Nine branded native providers plus approved Custom and AWS SageMaker. Real pre-save Test Connect now uses pinned OSS LiteLLM1.100.0 through authenticated private routing; status-based Save gating, five-minute expiry and edit/stale-result invalidation. SageMaker uses actual SDK SigV4/event-stream adapters, scoped installation IAM/model bindings and migration0145; cloud qualification remains pending. Paper `70046bcb`, artifact/coverage `46228c92`. Validation:26 SDK tests,1354 web tests, final focused UI/TypeScript/build, both API builds, both-edition focused DB/HTTP races, deterministic codegen and Compose/Helm checks pass. Independent security findings corrected and re-reviewed. Actual local CP/UI at `http://127.0.0.1:5180/agents/ai-gateway`, isolated `tunnexaiwalk0907repro4`, schema145 clean: real SDK→CONNECT→synthetic model preflight failed/succeeded correctly; tested connection74dc1255-2ea4-4571-b5ad-a93b3b58c8e4 applied revision1. Existing providers/team policies retained. SUBSEQUENT APPROVED LIVE WALK: OpenRouter pre-save inference passed HTTP200/statussuccess in5647ms, modelopenrouter/openai/gpt-4o-mini, max16tokens/no SDK retries; sanitized evidence committed. No connection saved; billing amount unmeasured. Read-only local STS subsequently verified account132613841543; this differs from prior sandbox735391218823. No SageMaker inventory/invocation. User designated local CP and ap-south (assumed ap-south-1); intended account and existing endpoint/IAM binding remain unresolved. Latest UI fix: editable Custom/SageMaker API base URL with approved-endpoint resolution and fixed-footer Test Connect; typed-URL local fixture test succeeded, endpoint edits invalidated success and unapproved URLs disabled Test/Save. Evidence `docs/AI-endpoint-entry-validation-20260907.md`. Prior binaries/engine state retained. Full composite gates INCONCLUSIVE from VM capacity; new image build and exact-head remote CI unperformed. No push/merge/release approval. Evidence `docs/AI-litellm-validation-20260907.md`; full-parity gaps `docs/AI-litellm-coverage.md`. LATEST USER STEERING: actual LiteLLM source reviewed; All Models now primary, full-width inline Add Model TAB per latest screenshots (creation drawer retired), existing-key reuse/automatic credential naming, read-only mapping preview/chat mode, LLM Credentials secondary with edit drawer retained, native API URLs visible; compact tables show logos; tab/cancel clears draft keys and rejects late results;22 focused and1354 full web tests/typecheck/build pass with local synthetic Test Connect proof. Azure Foundry remains unimplemented: SDK preflight alone cannot fix Bifrost saved-inference URL/auth. New private routing/authentication contract surfaced for disposition; evidence `docs/AI-litellm-model-flow-reference-20260907.md`. LATEST LOCAL FIX: direct Add Credentials currently requires model scope; initial Upstream API Base visible; explicit Custom endpoint action; Existing Credentials hides key AND endpoint, None clears key/test proof.27 focused and1359 full web tests/typecheck/build pass; local synthetic preflight and rendered saved-key hiding verified. Evidence `docs/AI-credential-entry-validation-20260907.md`. NEW REQUESTED SCOPE: independent credentials modal, multiple modes, broader provider registry and sanitized model detail. These remain unimplemented. Concrete D1-D3 state/protocol recommendations in `docs/S-AI-credential-modes-decisions.md` await disposition. NEXT ACTION: obtain disposition for inactive/untested independent credential storage and mode/provider routing contracts, then implement D1 with generated API and native-engine proof; full LiteLLM parity is NOT complete. Earlier checkpoints below are historical.**

**Companion-head correction (2026-09-09): client PR #6 now points to `9e968b85098410e1c73df0137e6338cdb5d1c7f9`, superseding the client head below. Its first Linux CI job found a pure serializer hidden by platform tags; the unchanged formatter was extracted to a shared file. Linux vet/tests, native Mac helper tests and Darwin/Windows cross-builds PASS; fresh client CI is running. Server content tip remains `d205e960cd4c67d18e8ff0462aff011a780694d6` (pre-merge), PR #66. No new server product content or live-runtime change.**

**AUTHORITATIVE CURRENT (2026-09-09): NAT managed relay integration — draft PR #66, content tip `d205e960cd4c67d18e8ff0462aff011a780694d6` (pre-merge), branch `codex/nat1-session-contract`, NOT MERGED. Companion tunnex-client draft PR #6, head `d77e39e208e52a054d8deb5eb3947fe043efc3a3`, product `24b66e1`. Server product `82d1b48`; local generate/migration/both-edition tests and builds/node/CLI/full web gates PASS. Client main321 and CI-scoped renderer64 tests/typechecks/builds PASS. Story-end findings were dispositioned; latest approved active-dial polling and tenant-scope corrections have clean bounded re-reviews. Read `docs/NAT-final-gates-20260909.md` and `docs/NAT-implementation-handoff.md`. Successful native AWS TURN-restart recovery proof used server `cddcd57` and client `55f4267`, not the subsequent narrow corrections. Exact PR-head CI and customer relay deployment/readiness, native Windows and named routing/MTU/lifecycle qualification remain outstanding; NOT beta-complete or merge-ready. Next: exact-head CI, then remaining qualification; do not restart unchanged proven Mac walks or broad reviews. Fresh merge/release approval required. Older checkpoints below are historical.**

**AUTHORITATIVE CURRENT (2026-09-06): Release draft visibility repair — PR #60, content tip `db215919a6ccbc446dd74b76a82a9224be87d947` (pre-merge); branch `codex/fix-release-draft-permissions`, NOT MERGED. Baseline `231f63f` is merged BYODB PR #59. Main publication failed because the publish job could not read its draft release with contents:read; the fix scopes contents:write to that existing push-only job and disables checkout credential persistence. Source/draft/ledger guards remain unchanged. Permission-aware baseline-red/fix-green contracts and two independent reviews pass; see `docs/S-release-draft-permissions-decisions.md`. Exact-final PR CI and fresh merge approval remain required. Live GitHub-token qualification is the next approved main publication, not the offline fixture. No release or cleanup performed. BYODB production-signed installer/upgrade walk remains deferred to its first signed release before availability claims or launcher-pin advancement. Earlier checkpoints below are historical.**
