...
**ACTIVE SITE-TO-SITE (2026-09-24): `story/site-to-site-connectivity`, isolated checkout `/private/tmp/tunnex-site-to-site-dev`, baseline `1a81f8fc`. Inventory/pair review `8838a17f` and reported diagnostics `71738b22` pushed. Fifth increment adds organization-wide served transit-hub roles using existing hub-set GET, with explicit failed/empty/unresolved/revoked states and no inferred pair path. Final 27 focused tests, full 1,573 web tests, typecheck and build pass; independent static review reports no actionable findings; desktop synthetic preview checked. Founder prioritizes desktop functionality. Synthetic preview `http://127.0.0.1:5198/.s2s-preview/index.html` stays untracked. No PR exists; required exact-head PR CI has not run. Feature-branch pushes authorized; no merge/release/CP or cloud deployment. Original dirty checkout preserved. NEXT: user visual acceptance and authenticated application/traffic proof remain separate; before IPsec backend work, settle the new ownership, runtime, secret and permission/entitlement decisions. See [decision/evidence ledger](docs/S-S2S-1-decisions.md). Earlier checkpoints below are historical.**
