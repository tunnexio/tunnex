...
**ACTIVE LOCAL (2026-09-07): AI-0 engine qualification on `ai-improvement`, content tip `5509c6ab`, decision paper `70741779`, based on main `5199b62d`; NOT PUSHED OR MERGED. Pinned Bifrost v2.0.0 experimental adapter/native harness and two authorized OpenRouter smoke responses are recorded in `docs/AI-0-qualification-20260907.md`. Final local race tests/vet pass, but four P2 review findings are HELD for disposition (timeouts, streaming assertions, restart control, exact denial contracts). AI-0 is incomplete: synthetic identity is not enrolled-agent proof; accounting/budget and broader engine-fit qualification remain. Next action: disposition the four findings, recommended fix all, then re-review. No production AI endpoint or NAT changes. Earlier checkpoints below are historical.**
