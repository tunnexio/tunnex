...
**ACTIVE LOCAL (2026-09-07): AI gateway AI-0–AI-5, usage/configuration and multi-provider onboarding on `ai-improvement`, content tip `69ddefda6540d0c3da6b25a8410a3bde10da29d1`, baseline main `26a36afc`; NOT PUSHED OR MERGED. Multi-provider paper `e46a9255`, OpenAPI contract `c9586549`. Community default-OFF; OpenAI, Anthropic, Gemini and OpenRouter now use direct native adapters with provider-bound keys/models. UI has a themed right drawer, backend provider picker, catalog search, model inventory and saved-credential model additions. Migration 0142 preserves 0141 ownership; legacy ACL covers OpenRouter only. Stable virtual-key and retained provider-config IDs verified through scope changes and native restart. 1,339 web tests/typecheck/build, both API builds, focused provider/policy PostgreSQL races in both editions, HTTP auth/schema races, strict core-four native JSON/SSE/rotation/refusal/restart suite, CLI and deterministic generated-code checks PASS. Full API gates remain INCONCLUSIVE from the Docker VM capacity blocker; exact-head remote CI has not run. Validation/review evidence: `docs/AI-multiprovider-validation-20260907.md`. Actual CP/UI RUNNING at `http://127.0.0.1:5180/agents/ai-gateway` on isolated `tunnexaiwalk0907repro4`, schema142 clean. Models tab shows two models on the existing synthetic demo credential; original team selections unchanged, usage remains8 requests/56 tokens/$0.000170. No paid-provider calls in this slice. Azure/Bedrock/Vertex/custom origins/aliases and real new-provider account smoke remain separate qualification. Website remains local `621b8d7`; unrelated work/stashes preserved. NEXT ACTION: obtain approval to resolve Docker capacity (8→16 GiB requires shared local SSO restart), then rerun full required gates before separately approved push/PR and exact-head CI. No merge/release authorization. Earlier checkpoints below are historical.**
