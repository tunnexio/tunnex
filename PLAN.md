...
**ACTIVE LOCAL (2026-09-07): AI gateway AI-0–AI-5 and provider UI on `ai-improvement`, content tip `2744bf439a80c20cb13e96e11a9ce6733079463d`, baseline main `26a36afc`; NOT PUSHED OR MERGED. Community default-OFF. Nine branded native providers (OpenAI, Anthropic, Gemini, OpenRouter, Groq, Mistral, Cerebras, xAI, DeepSeek) plus opt-in Custom HTTP/HTTPS with mandatory authenticated egress proxy and installation endpoint/CIDR allowlist. Custom namespacing/immutable endpoints and migrations0143/0144 preserve ownership. Actual logos, searchable dropdown, compact model suggestions/chips/manual entry and existing-credential reuse are implemented. Saved Test connection is catalog access; public catalogs may not validate keys. USER REQUEST STILL OPEN: Test connection BEFORE Add is NOT implemented; transient zero-inference ten-second probe design is surfaced and awaiting user disposition (see latest decision paper). Paper references `7b34ecd7`, `6b7beab3`, `1255a79b`, `9cf753c4`, `1417fd81`; OpenAPI contract `7737a97a`. Final1348 web tests/typecheck/build, both API builds, focused provider/policy PostgreSQL races in both editions, HTTP auth/schema races, expanded native JSON/SSE/custom-proxy refusal/restart suites, CLI, Compose/Helm and deterministic codegen pass. Full composite gates remain INCONCLUSIVE from Docker VM capacity; exact-head remote CI has not run. Validation/review: `docs/AI-custom-providers-validation-20260907.md`. Actual CP/UI RUNNING at `http://127.0.0.1:5180/agents/ai-gateway` on isolated `tunnexaiwalk0907repro4`, schema144 clean; private fixture connection applied and catalog check succeeds through production proxy. Original OpenRouter fixture rev7 and team policies preserved, usage8 requests/56 tokens/$0.000170. Previous engine container retained stopped/disconnected, state volumes preserved. No paid calls, shared SSO restart, cloud action or cleanup. Website remains local `621b8d7`; unrelated work/stashes preserved. NEXT ACTION: resolve pending pre-save check design, then implement OpenAPI/probe/UI success-invalidation flow; do not claim requested onboarding complete before that. No merge/release authorization. Earlier checkpoints below are historical.**
