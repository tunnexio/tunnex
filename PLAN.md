...
**ACTIVE LOCAL (2026-09-07): AI gateway AI-0–AI-5 plus user-requested usage/configuration dashboard refinement on `ai-improvement`, content tip `b90aac8df68b8d067b43dc8c6b6362ca07ab8f1a`, baseline main `26a36afc`; NOT PUSHED OR MERGED. Community default-OFF, explicit AI team/narrowing policy and honest soft thresholds remain implemented. Usage now shows native scoped daily spend, request/token metrics and historical team/agent/model rankings; Configuration has matching scoped styling and readable decimal thresholds (11 affected policy tests pass); provider-secret onboarding remains operator configuration outside this UI. Full API suites/builds pass in both editions, actual pinned native dashboard race proof passes, 1,325 web tests/typecheck/build and CLI tests pass, codegen second-run drift is zero. Prior installed identity/rotation proofs remain in `docs/AI-2-5-validation-20260907.md`; new results/limits and live preview details are in `docs/AI-usage-dashboard-validation-20260907.md`. Local CP/UI intentionally RUNNING for user review at `http://127.0.0.1:5180/agents/ai-gateway` using isolated `tunnexaiwalk0907repro4` test data, no paid provider calls. Website guide remains local `621b8d7` at `/private/tmp/tunnex-web-ai-improvement`; original dirty website checkout/stash preserved. User visual approval and required exact-head remote CI remain pending. NEXT ACTION: collect visual review, then obtain push/PR authorization and run required GitHub checks on the exact head. No merge/release authorization. Earlier checkpoints below are historical.**
