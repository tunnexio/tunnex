...
**ACTIVE LOCAL (2026-09-07): AI gateway AI-0–AI-5 plus usage/configuration and secure provider onboarding on `ai-improvement`, content tip `3d988133499ed483ac6bb29bb7b6910edee909fa`, baseline main `26a36afc`; NOT PUSHED OR MERGED. Provider paper `8e5af9e7`, OpenAPI contract `283c40c7`. Community default-OFF; OpenRouter Providers & models UI now supports write-only encrypted-engine key setup, cached model search, no-inference credential tests, rotation, disable and safe deletion, with owned team selection and migration 0141 legacy ACL. Actual local CP walk verified create/test/team-save/reference-delete refusal/rotation/engine restart; original team key selection restored, demo connection remains applied. 1,335 web tests/typecheck/build, both API builds, focused provider/policy PostgreSQL race suites in both editions, native lifecycle race proof, CLI, static Compose/Helm and committed codegen drift checks PASS. Full API suite is INCONCLUSIVE due to the 8 GiB Docker VM ENOSPC; no full gate or NetBird-wide parity claim. Private backups of only our retired preview binaries and exact failed test-fixture cleanup recovered local preview, leaving unrelated SSO containers/data untouched. Validation and review dispositions: `docs/AI-provider-onboarding-validation-20260907.md`; verified competitive gaps: `docs/S-AI-provider-onboarding-decisions.md`. Local CP/UI intentionally RUNNING at `http://127.0.0.1:5180/agents/ai-gateway` on isolated `tunnexaiwalk0907repro4`; synthetic keys only, usage unchanged (8 requests/56 tokens/$0.000170). Website remains local `621b8d7`; unrelated root/website work and stashes preserved. NEXT ACTION: obtain approval to expand the local Docker VM 8→16 GiB (shared local SSO restart), then rerun required full gates before push/PR and exact-head CI. No merge/release authorization. Earlier checkpoints below are historical.**
