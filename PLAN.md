...
**AUTHORITATIVE CURRENT (2026-09-09): CLI package distribution — PR #65, content tip `6bdbf79bd717bf3839ed06d15657aa20796547a3` (pre-merge), branch `tunnex-cli-publish`.** First-party Homebrew, signed Linux repositories, Nix/Arch recipes and `https://get.tunnex.io/cli` are published with committed native/public verification under `walk-artifacts/cli-publish/`. Distribution implementations live in `tunnexio/homebrew-tap` and `tunnexio/packages`; the website serves the CLI installer. This PR preserves reviewed snapshots and evidence, not control-plane runtime changes. Official archive submissions and Linux DNS-helper expansion remain pending. Earlier checkpoints below are historical.
