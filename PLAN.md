...
**AUTHORITATIVE CURRENT (2026-09-06): CP BYODB — PR #59, content tip `7587b26e11872e2942631044b033b5c6f7bbb903` (pre-merge), product implementation `09e1ce4`; branch `codex/byodb-private-postgresql`, NOT MERGED. Provider-neutral PostgreSQL 16/17/18, private VM/RDS/Neon runtime, preserved migration locks, fail-closed required channel binding, native backup trust parity, bounded startup, and Kubernetes mTLS/separate-role/manual-rotation proof are recorded in `walk-artifacts/BYODB/completion.md` and `kubernetes-mtls.md`. Same-PR CI performance scope now parallelizes gate lanes with isolated DBs/build caches and a fail-closed final check; see `walk-artifacts/BYODB/ci-performance.md`. Exact-head timing/gates remain pending. Customer docs: tunnex-web PR #40. Exact-final required CI and fresh in-session merge approval remain mandatory. User explicitly deferred production-signed installer/upgrade wire qualification to the first BYODB-capable signed release, before public availability or launcher-pin advancement; no private-distribution mode or trust bypass. Retain isolated test resources and protected scratch credentials; no cleanup or merge based on prior approvals. Earlier checkpoints below are historical.**
