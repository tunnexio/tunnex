...
**ACTIVE SITE-TO-SITE (2026-09-24): `story/site-to-site-connectivity`, isolated checkout `/private/tmp/tunnex-site-to-site-dev`, baseline `1a81f8fc`. Inventory/pair review `8838a17f` and diagnostics paper `684ef064` pushed. Diagnostics increment reuses existing reported health projections; full 1,565 web tests pass. The reviewed mobile P2 was fixed with scoped wrapping: previously 411px, now 390px document at 390px viewport; independent re-review, final 19 affected tests, typecheck and build pass. Founder prioritizes desktop functionality. Reuses existing reads only; no connection persistence or IPsec runtime. Synthetic preview `http://127.0.0.1:5198/.s2s-preview/index.html` stays untracked. No PR exists; required exact-head PR CI has not run. Feature-branch pushes authorized; no merge/release/CP or cloud deployment. Original dirty checkout preserved. NEXT: publish diagnostics and continue served transit-hub visibility using existing read contracts; user visual acceptance remains separate. IPsec state-model/runtime/permission choices remain deferred. See [decision/evidence ledger](docs/S-S2S-1-decisions.md). Earlier checkpoints below are historical.**
