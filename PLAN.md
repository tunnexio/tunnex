...
**ACTIVE LOCAL (2026-09-07): AI gateway AI-0–AI-5, provider UI and LiteLLM SDK integration on `ai-improvement`, content tip `313af00263bf02dcacc8692deaa9dbabebbc86e3` (implementation `3fdddc20`), baseline main `26a36afc`; NOT PUSHED OR MERGED. Community default-OFF. Nine branded native providers plus approved Custom and AWS SageMaker. Real pre-save Test Connect now uses pinned OSS LiteLLM1.100.0 through authenticated private routing; status-based Save gating, five-minute expiry and edit/stale-result invalidation. SageMaker uses actual SDK SigV4/event-stream adapters, scoped installation IAM/model bindings and migration0145; cloud qualification remains pending. Paper `70046bcb`, artifact/coverage `46228c92`. Validation:26 SDK tests,1352 web tests, final focused UI/TypeScript/build, both API builds, both-edition focused DB/HTTP races, deterministic codegen and Compose/Helm checks pass. Independent security findings corrected and re-reviewed. Actual local CP/UI at `http://127.0.0.1:5180/agents/ai-gateway`, isolated `tunnexaiwalk0907repro4`, schema145 clean: real SDK→CONNECT→synthetic model preflight failed/succeeded correctly; tested connection74dc1255-2ea4-4571-b5ad-a93b3b58c8e4 applied revision1. Existing providers/team policies retained. SUBSEQUENT APPROVED LIVE WALK: OpenRouter pre-save inference passed HTTP200/statussuccess in5647ms, modelopenrouter/openai/gpt-4o-mini, max16tokens/no SDK retries; sanitized evidence committed. No connection saved; billing amount unmeasured. No AWS calls. Real SageMaker walk still needs designated CP host, existing endpoint/region and IAM configuration. Prior binaries/engine state retained. Full composite gates INCONCLUSIVE from VM capacity; new image build and exact-head remote CI unperformed. No push/merge/release approval. Evidence `docs/AI-litellm-validation-20260907.md`; full-parity gaps `docs/AI-litellm-coverage.md`. NEXT ACTION: obtain designated CP host and existing SageMaker endpoint/region to perform the approved live SageMaker walk; then extend imported117-provider metadata into real routing families; full LiteLLM parity is NOT complete. Earlier checkpoints below are historical.**
