...
**ACTIVE LOCAL (2026-09-07): AI gateway foundation on `ai-improvement`, content tip `4c821c26532f5eee9b4df8964a163eec0150ef52`, AI-1 paper `72b92b3d`, based on main `5199b62d`; NOT PUSHED OR MERGED. AI-0 pinned Bifrost native/enrolled OpenRouter proofs and user-approved soft-threshold limits are recorded in `docs/AI-0-qualification-20260907.md` and `docs/AI-0-budget-qualification.md`. Community default-off credential persistence, bounded streaming/JSON errors, private engine client, sealed scoped keys, HTTP/RBAC seams, UI setting and deployment prototype are committed. Full API tests/builds in BOTH editions, Linux node tests, 1300 web tests/typecheck/build, native AI race tests and 49-file codegen drift check pass; see `docs/AI-1-validation-20260907.md`. Epic INCOMPLETE: production composition remains unavailable until AI-2 policy resolver; reconciliation, scoped usage/threshold integration, customer install/Helm/public docs and final live/remote CI qualification remain. Next action: obtain the pending team-composition disposition in `docs/S-AI-2-decisions.md` (recommended one explicit AI team per agent with narrowing overrides), freeze that contract, then parallelize its implementation. Existing unrelated worktrees/stashes preserved. No merge/release authorization. Earlier checkpoints below are historical.**
