...
import { Button, Card, EmptyState, ErrorText, Field, Input, Loading, Modal, Select } from "./ui";
...
export function PrivateDNSResolvers({ orgId, role }: { orgId: string; role: Member["role"] | undefined }) {
  ...
  return <Card><section aria-labelledby="private-dns-heading" className="space-y-4">
    ...
    {canManage && config && <button className="text-xs text-danger hover:underline" onClick={() => setDialog("delete")}>Remove resolver</button>}
  </section></Card>;
}
