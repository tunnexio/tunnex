...
export function PrivateDNSResolvers({ orgId, role }: { orgId: string; role: Member["role"] | undefined }) {
  ...
  return <Card><section aria-labelledby="private-dns-heading" className="space-y-3">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 id="private-dns-heading" className="text-lg font-semibold text-ink-heading">Private DNS resolvers</h2><p className="mt-1 text-sm text-ink-tertiary">One path, multiple DNS zones · most-specific zone wins</p></div>{canManage && config && <Button size="sm" variant="ghost" onClick={openConfigure}>Edit profiles</Button>}</div>
    {inventoryError ? <LoadRetry error={"Could not load Sites and Gateways: " + inventoryError} onRetry={() => void loadInventory()} /> : sites === null || nodes === null ? <Loading label="Loading resolver paths…" /> : <><div className="grid max-w-4xl gap-2 sm:grid-cols-2"><Field label="Site"><Select value={siteId} onChange={(event) => setSiteId(event.target.value)}><option value="">Select a Site</option>{sites.map((site) => <option key={site.id} value={site.id}>{site.name}</option>)}</Select></Field><Field label="Gateway"><Select value={gatewayId} disabled={!siteId} onChange={(event) => setGatewayId(event.target.value)}><option value="">Select a Gateway</option>{gateways.map((gateway) => <option key={gateway.id} value={gateway.id}>{gateway.name}</option>)}</Select></Field></div>
      {!siteId || !gatewayId ? <p className="rounded-md border border-line px-3 py-2 text-sm text-ink-tertiary">Select an active Site and Gateway.</p> : configLoading ? <Loading label="Loading private DNS resolvers…" /> : configError && dialog === null ? <LoadRetry error={configError} onRetry={() => void loadConfig()} /> : config ? <div className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-2 text-xs text-ink-tertiary"><span>{selectedSite?.name} · {selectedGateway?.name} · active version {config.version}</span><span className="text-ok">Active</span></div>{configProfiles.map((profile) => <article key={profile.id} className="border-t border-white/10 py-3 first:border-t-0 first:pt-0 last:pb-0"><div className="flex items-start gap-3"><ProviderMark provider={profile.provider_hint} /><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-semibold text-ink-heading">{profile.name}</p><p className="text-xs text-ink-tertiary">{providerName(profile.provider_hint)}</p></div><span className="text-xs text-ink-tertiary">{profile.legacy_default ? "Legacy catch-all" : profile.zone_suffixes.join(" · ")}</span></div><ul className="mt-2 flex flex-wrap gap-2">{profile.endpoints.map((endpoint) => <li key={endpoint.address+endpoint.port+endpoint.transport} className="rounded-inset border border-white/10 px-2 py-1 font-mono text-xs text-ink-body">{endpoint.address}:{endpoint.port} · {endpoint.transport.toUpperCase()}</li>)}</ul></div></div></article>)}</div> : missing ? <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-line px-3 py-2"><p className="text-sm text-ink-tertiary">No resolver profiles for {selectedSite?.name} · {selectedGateway?.name}.</p>{canManage && <Button size="sm" onClick={openConfigure}>Configure profiles</Button>}</div> : null}</>}
