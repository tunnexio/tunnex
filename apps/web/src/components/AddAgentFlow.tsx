...
export function AddAgentFlow({ orgId, enabled = false, runtimeEnabled = true, onDismiss, visualStage }: { orgId: string; enabled?: boolean; runtimeEnabled?: boolean; onDismiss: () => void; visualStage?: AddAgentVisualStage }) {
  ...
  if (shownStage === "review") return <Modal title="Step 2 of 3 — review bootstrap" onDismiss={dismiss} actions={<><Button variant="ghost" onClick={() => setStage("details")}>Back</Button><Button disabled={busy} onClick={() => void issue()}>{busy ? "Issuing…" : "Issue one-time command"}</Button></>}><div className="space-y-3"><p className="text-sm text-ink-secondary">Confirm the identity and gateway before creating a single-use token. This does not enroll an agent.</p><p className="text-sm text-ink-secondary">Before running the installer, enable runtime synchronization in <a className="text-accent-400" href="/settings?section=ai-agents">AI Agent settings</a> (requires a paid plan). The agent host must resolve and reach this control plane over HTTPS. {AGENT_PREREQ}</p><dl className="divide-y divide-white/10 rounded-md border border-white/10 text-sm"><div className="flex justify-between gap-4 px-3 py-2"><dt className="text-ink-tertiary">Agent name</dt><dd className="font-medium text-ink-heading">{name.trim()}</dd></div><div className="flex justify-between gap-4 px-3 py-2"><dt className="text-ink-tertiary">Gateway</dt><dd className="font-medium text-ink-heading">{gateway?.name ?? "Selected gateway"}</dd></div></dl><ErrorText>{error}</ErrorText></div></Modal>;
