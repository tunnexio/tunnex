...
function ProviderWorkspace({ orgId, canManage = true, view: routeView, onViewChange, onGrantAccess }: WorkspaceProps) {
  ...
  async function save(body: S["AIProviderCreate"] | S["AIProviderUpdate"], connection?: AIProviderConnection) {
    ...
    if (ok && saved?.models.length) {
      ...
      else setConnectionModel(saved.models[0]);
    }
  }
  ...
  const credentialSelection = credentialRows.filter(c => selectedCredentials.includes(c.id));
  const credential = credentialSelection.length === 1 ? credentialSelection[0] : undefined;
  const credentialActionsDisabled = busy || !canManage || !inventory?.management_available;
  const selectedRows = modelRows.filter(({ connection, model }) => selectedModels.includes(`${connection.id}:${model}`));
  const single = selectedRows.length === 1 ? selectedRows[0] : undefined;
  const selectedConnections = [...new Map(selectedRows.map(row => [row.connection.id, row.connection])).values()];
  const allSelected = modelRows.length > 0 && selectedRows.length === modelRows.length;
  const selectionToolbar = <div className="ai-model-selection-toolbar" aria-label="Selected model actions"><span>{selectedRows.length} selected</span><div className="ai-inventory-actions">
    <Button size="sm" disabled={!single || busy} onClick={() => single && setConnectionModel(single.model)}>Use model</Button>
    {onGrantAccess && <Button size="sm" disabled={!single || busy || !canManage || !single.connection.enabled || single.connection.status !== "applied" || single.connection.revision !== single.connection.applied_revision} onClick={() => single && onGrantAccess(single.connection.id, single.model)}>Grant access</Button>}
    <Button size="sm" disabled={!single || busy || !canManage || !inventory?.management_available} onClick={() => single && setEditing(single.connection)}>Edit credentials</Button>
    <Button size="sm" disabled={!selectedRows.length || busy || !canManage || !inventory?.management_available} onClick={async () => {
      for (const c of selectedConnections) {
        const ok = await mutate(() => api.POST("/api/v1/organizations/{orgId}/ai-gateway/providers/{connectionId}/test", { params: { path: { orgId, connectionId: c.id } }, body: { expected_revision: c.revision } }), "Catalog check completed.");
        if (!ok) break;
      }
    }}>Check catalog</Button>
    <HelpTooltip label="About model selection">Use model, Grant access and Edit credentials require one selected model. Catalog checks run once per selected credential.</HelpTooltip>
  </div></div>;
  return <section className="ai-provider-workspace" aria-label="Models and endpoints">
    ...
    {loading && !inventory ? <p role="status">Loading provider connections…</p> : !inventory ? <Button onClick={() => { setError(""); void reload(); }}>Retry provider connections</Button> : <>
      ...
      {view !== "connections" && <div className="ai-provider-panel ai-provider-model-table"><div className="ai-provider-model-filters"><Field label="Search configured models"><Input value={modelSearch} onChange={(e) => setModelSearch(e.target.value)} placeholder="Model ID or credential name" /></Field><Field label="Filter models by provider"><select value={modelProvider} onChange={(e) => setModelProvider(e.target.value)}><option value="">All providers</option>{[...new Set(inventory.items.map((c) => c.provider))].map((id) => <option key={id} value={id}>{providerName(id)}</option>)}</select></Field></div>{selectionToolbar}<div className="ai-provider-table-scroll"><table><caption className="sr-only">Configured models</caption><thead><tr><th><input type="checkbox" aria-label="Select all visible models" checked={allSelected} ref={input => { if (input) input.indeterminate = selectedRows.length > 0 && !allSelected; }} onChange={e => setSelectedModels(e.target.checked ? modelRows.map(({ connection, model }) => `${connection.id}:${model}`) : [])} /></th><th>Model</th><th>Provider</th><th>Mode</th><th>Credential</th><th>State</th></tr></thead><tbody>{modelRows.map(({ connection: c, model }) => <tr key={`${c.id}:${model}`}><td><input type="checkbox" aria-label={`Select ${model}`} checked={selectedModels.includes(`${c.id}:${model}`)} onChange={e => setSelectedModels(previous => e.target.checked ? [...previous, `${c.id}:${model}`] : previous.filter(key => key !== `${c.id}:${model}`))} /></td><th scope="row"><button className="ai-item-button" title={model} onClick={() => setConnectionModel(model)}>{modelDisplayName(model)}</button></th><td><span className="ai-provider-table-brand"><ProviderLogo provider={c.provider} />{providerName(c.provider)}</span></td><td title={modeLabel(c.model_modes?.[model] ?? "chat")}>{modeLabel(c.model_modes?.[model] ?? "chat").split(", ")[0]}</td><td>{c.name}</td><td><Badge tone={c.status === "error" ? "danger" : c.status === "pending" ? "warn" : "neutral"}>{c.status}</Badge></td></tr>)}</tbody></table></div>{!modelRows.length && <p className="ai-provider-empty">No configured models match this view. Add a model or adjust your filters.</p>}</div>}
      {view === "connections" && <div className="ai-provider-panel ai-provider-model-table ai-credential-table">
        <div className="ai-provider-model-filters"><Field label="Search credentials"><Input value={credentialSearch} onChange={e => setCredentialSearch(e.target.value)} placeholder="Credential name or provider" /></Field><Field label="Filter credentials by provider"><select value={credentialProvider} onChange={e => setCredentialProvider(e.target.value)}><option value="">All providers</option>{[...new Set(inventory.items.map(c => c.provider))].map(id => <option key={id} value={id}>{providerName(id)}</option>)}</select></Field></div>
        <div className="ai-model-selection-toolbar"><span>{credentialSelection.length} selected</span><div className="ai-inventory-actions">
          <Button size="sm" disabled={credentialActionsDisabled || !credential} onClick={() => credential && setEditing(credential)}>Edit credentials</Button>
          <Button size="sm" disabled={credentialActionsDisabled || !credentialSelection.length} onClick={async () => { for (const c of credentialSelection) { if (!await mutate(() => api.POST("/api/v1/organizations/{orgId}/ai-gateway/providers/{connectionId}/test", { params: { path: { orgId, connectionId: c.id } }, body: { expected_revision: c.revision } }), "Catalog check completed.")) break; } }}>Check catalog</Button>
          <Button size="sm" disabled={credentialActionsDisabled || !credential} onClick={() => { if (!credential) return; const c = credential; void mutate(() => api.PUT("/api/v1/organizations/{orgId}/ai-gateway/providers/{connectionId}", { params: { path: { orgId, connectionId: c.id } }, body: { provider: c.provider, ...(c.endpoint_url ? { endpoint_url: c.endpoint_url } : {}), name: c.name, models: c.models, enabled: !c.enabled, expected_revision: c.revision } }), "Credential state updated."); }}>{credential?.enabled === false ? "Enable" : "Disable"}</Button>
          <Button size="sm" disabled={credentialActionsDisabled || !credential} onClick={() => credential && setRemoving(credential)}>Delete</Button>
          <HelpTooltip label="About credential actions">Select one credential to edit, enable, disable or delete it. Catalog checks support multiple selections. Editing or disabling affects every model using the credential; disabling blocks new requests while accepted streams may finish within 30 seconds.</HelpTooltip>
        </div></div>
        <div className="ai-provider-table-scroll"><table><caption className="sr-only">Saved LLM credentials</caption><thead><tr><th><input type="checkbox" aria-label="Select all visible credentials" checked={credentialRows.length > 0 && credentialSelection.length === credentialRows.length} ref={input => { if (input) input.indeterminate = credentialSelection.length > 0 && credentialSelection.length < credentialRows.length; }} onChange={e => setSelectedCredentials(e.target.checked ? credentialRows.map(c => c.id) : [])} /></th><th>Credential</th><th>Provider</th><th>Models</th><th>State</th><th>Catalog check</th></tr></thead><tbody>{credentialRows.map(c => <tr key={c.id}>
          <td><input type="checkbox" aria-label={`Select credential ${c.name}`} checked={selectedCredentials.includes(c.id)} onChange={e => setSelectedCredentials(ids => e.target.checked ? [...ids, c.id] : ids.filter(id => id !== c.id))} /></td>
          <th scope="row"><button className="ai-item-button" onClick={() => setCredentialDetails(c)}>{c.name}</button></th>
          <td><span className="ai-provider-table-brand"><ProviderLogo provider={c.provider} />{providerName(c.provider)}</span></td>
          <td><button className="ai-item-button" aria-label={`View models for ${c.name}`} onClick={() => setCredentialDetails(c)}>{c.models.length}</button></td>
          <td><Badge tone={c.status === "error" ? "danger" : c.status === "pending" ? "warn" : "neutral"}>{c.enabled ? c.status : "Disabled"}</Badge></td>
          <td title={c.last_test_at ? new Date(c.last_test_at).toLocaleString() : "No catalog check recorded"}><Badge tone={c.last_test_status === "failed" ? "danger" : "neutral"}>{c.last_test_status}</Badge></td>
        </tr>)}</tbody></table></div>
        {!credentialRows.length && <p className="ai-provider-empty">{inventory.items.length ? "No credentials match your filters." : "No saved credentials yet."}</p>}
      </div>}
      {credentialDetails && <Modal title={credentialDetails.name} showClose onDismiss={() => setCredentialDetails(null)} actions={<Button onClick={() => setCredentialDetails(null)}>Close</Button>}><div className="ai-detail-stack"><span className="ai-provider-table-brand"><ProviderLogo provider={credentialDetails.provider} />{providerName(credentialDetails.provider)}<HelpTooltip label="Credential identifiers">{credentialDetails.key_id}. Desired revision {credentialDetails.revision}, applied {credentialDetails.applied_revision}.</HelpTooltip></span><div><h3>Models</h3><ul className="ai-credential-models">{credentialDetails.models.map(model => <li key={model} title={model}>{modelDisplayName(model)}</li>)}</ul></div></div></Modal>}
      ...
      <div className="text-right"><HelpTooltip label="About catalog checks">Catalog checks generate no model tokens. Public catalogs may not validate API keys. A successful catalog check does not prove inference access.</HelpTooltip></div>
      {removing && <Modal title={`Delete ${removing.name}?`} danger onDismiss={() => !busy && setRemoving(null)}>{error && <p role="alert">{error}</p>}<p>Remove this connection from every team policy first. The server refuses deletion while references remain. Usage history is preserved. These credentials and all their configured models are removed.</p><div className="ai-provider-form-actions"><Button disabled={busy} onClick={() => setRemoving(null)}>Cancel deletion</Button><Button disabled={busy} onClick={async () => { const c = removing; if (await mutate(() => api.DELETE("/api/v1/organizations/{orgId}/ai-gateway/providers/{connectionId}", { params: { path: { orgId, connectionId: c.id } }, body: { expected_revision: c.revision } }), "Provider connection deleted. Usage history is retained.")) setRemoving(null); }}>Confirm deletion</Button></div></Modal>}
      {accessAfterSave && !grantAfterSave && <Modal title="Model saved · Set up access" showClose onDismiss={() => setAccessAfterSave(null)} actions={<><Button onClick={() => setAccessAfterSave(null)}>Skip</Button><Button onClick={() => setGrantAfterSave(true)}>Choose access</Button></>}><p>Grant access to a user group so its members can use this model in Playground. You can also do this later from Access.</p></Modal>}
...
function ProviderEditor({ draft, error, orgId, connection: initialConnection, definitions, supportedModes, testAvailable, publicEndpointsAvailable, foundryAvailable, foundryEndpoints, sagemakerAvailable, sagemakerEndpoints, customAvailable, endpoints, connections, modelOnly, busy, onSave, onCancel }: { draft?: EditorDraft; error: string; orgId: string; connection?: AIProviderConnection; definitions: Definition[]; supportedModes: ModelMode[]; testAvailable: boolean; publicEndpointsAvailable: boolean; foundryAvailable: boolean; foundryEndpoints: NonNullable<S["AIProviderList"]["foundry_endpoints"]>; sagemakerAvailable: boolean; sagemakerEndpoints: NonNullable<S["AIProviderList"]["sagemaker_endpoints"]>; customAvailable: boolean; endpoints: NonNullable<S["AIProviderList"]["custom_endpoints"]>; connections: AIProviderConnection[]; modelOnly: boolean; busy: boolean; onSave: (body: S["AIProviderCreate"] | S["AIProviderUpdate"], connection?: AIProviderConnection) => Promise<void>; onCancel: (draft: EditorDraft) => void }) {
  ...
  const steps = ["Provider", "Credentials", "Models", "Review"];
  ...
  async function search(offset = 0) {
    ...
    try {
      const r = draftCatalog
        ? await api.POST("/api/v1/organizations/{orgId}/ai-gateway/providers/model-catalog", { params: { path: { orgId } }, body: { provider, api_key: secret, endpoint_url: usesEndpoint ? selectedEndpoint : "", query: query.trim(), mode, limit: 50, offset } })
      ...
      if (r.error || !r.data) throw Error();
      const page = r.data;
      setCatalog(previous => offset > 0 && previous ? { ...page, items: [...previous.items, ...page.items.filter(item => !previous.items.some(existing => existing.id === item.id))] } : page);
  ...
  const editor = (
    ...
    {initialConnection ? <div className="ai-provider-fixed-brand"><ProviderLogo provider={initialConnection.provider} /><strong>{providerLabel}</strong><HelpTooltip label="About this provider">A connection's provider cannot be changed.</HelpTooltip></div> : <div className="ai-provider-picker"><EntityPicker label="Provider" value={provider ?? ""} placeholder="Select a provider" options={definitions.map((d) => ({ value: d.id, kind: "provider", tag: modelOnly && (d.id === "custom" && !customAvailable && !publicEndpointsAvailable || d.id === "sagemaker" && !sagemakerAvailable && !publicEndpointsAvailable || d.id === "azure_foundry" && !foundryAvailable && !publicEndpointsAvailable) ? "Setup required" : "", label: d.name, icon: <ProviderLogo provider={d.id} /> }))} onSelect={(option) => { const selected = definitions.find((d) => d.id === option.value); if (selected) switchProvider(selected.id); }} /></div>}
    {modelOnly && definitions.some((d) => d.id === "custom") && !customAvailable && !publicEndpointsAvailable && <p>Custom providers require an installation-approved endpoint and secure egress setup.</p>}
    ...
    {modelOnly && <><Field label="Existing Credentials"><select value={existingID} onChange={(e) => selectCredentials(e.target.value)}><option value="">None, enter new credentials below</option>{connections.filter((c) => (!provider || c.provider === provider) && (c.status === "applied" && c.applied_revision === c.revision || c.status === "disabled")).map((c) => <option key={c.id} value={c.id}>{c.name} · {definitions.find((d) => d.id === c.provider)?.name ?? c.provider} · {c.status}</option>)}</select></Field><HelpTooltip>Choose saved credentials to select their provider automatically, or enter new credentials below. Pending or failed credentials need their key re-entered in LLM Credentials before reuse.</HelpTooltip>{connection && <p>Using {connection.name}. Its API key stays private and existing models are preserved.</p>}</>}
    ...
    {wizard && <dl className="ai-wizard-review"><div><dt>Provider</dt><dd>{providerLabel}</dd></div><div><dt>Credential</dt><dd>{savedName}</dd></div><div><dt>Models</dt><dd>{finalModels.map(model => `${modelDisplayName(model)} (${modeLabel(modelMode(model)).split(", ")[0]})`).join(", ")}</dd></div><div><dt>Access</dt><dd>{enabled ? "Enabled" : "Disabled"}</dd></div></dl>}
  ...
  return <Modal title={modelOnly ? "Add Model" : connection ? "Edit credentials" : "Add Credentials"} size="wide" showClose onDismiss={dismiss} actions={actions}>{error && <p role="alert" className="ai-provider-alert">{error}</p>}{editor}</Modal>;
}
