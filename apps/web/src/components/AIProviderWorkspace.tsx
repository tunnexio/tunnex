...
function ProviderWorkspace({ orgId }: { orgId: string }) {
  ...
  return <section className="ai-provider-workspace" aria-label="Provider connections">
    ...
    {loading && !inventory ? <p role="status">Loading provider connections…</p> : !inventory ? <Button onClick={() => { setError(""); void reload(); }}>Retry provider connections</Button> : <>
      ...
      {editing && <ProviderEditor key={typeof editing === "string" ? "new" : `${editing.id}:${editing.revision}`} orgId={orgId} connection={typeof editing === "string" ? undefined : editing} definitions={definitions} connections={inventory.items} modelOnly={editing === "model"} busy={busy} onCancel={() => setEditing(null)} onSave={save} />}
      <Button disabled={loading || busy} onClick={() => { setError(""); void reload(); }}>Refresh providers</Button>
    </>}
  </section>;
}
function ProviderEditor({ orgId, connection: initialConnection, definitions, connections, modelOnly, busy, onSave, onCancel }: { orgId: string; connection?: AIProviderConnection; definitions: Definition[]; connections: AIProviderConnection[]; modelOnly: boolean; busy: boolean; onSave: (body: S["AIProviderCreate"] | S["AIProviderUpdate"], connection?: AIProviderConnection) => Promise<void>; onCancel: () => void }) {
  const [provider, setProvider] = useState<AIProviderConnection["provider"]>(initialConnection?.provider ?? definitions.find((d) => d.id === "openrouter")?.id ?? definitions[0]?.id ?? "openrouter");
  ...
  const providerLabel = definition?.name ?? initialConnection?.provider ?? "Provider unavailable";
  const formId = useId();
  ...
  async function search(offset = 0) {
    ...
    try {
      const r = await api.GET("/api/v1/organizations/{orgId}/ai-gateway/models", { params: { path: { orgId }, query: { provider, query: query.trim(), limit: 50, offset } } });
  ...
  function switchProvider(next: AIProviderConnection["provider"]) {
    ...
    setProvider(next); setSecret(""); setModels(""); setExistingID(""); setQuery(""); setCatalog(null); setCatalogError(""); setSearching(false); searchSerial.current++;
  }
  const chosen = uniqueLines(models);
  const finalModels = modelOnly && connection ? [...new Set([...connection.models, ...chosen])] : chosen;
  const valid = (!!connection || !!definition) && (connection && modelOnly || name.trim().length > 0 && name.trim().length <= 80) && chosen.length > 0 && finalModels.length <= 32 && chosen.every((m) => m.startsWith(`${provider}/`) && m.length > provider.length + 1 && m.length <= 255 && !/\s/.test(m)) && (connection && !secret || secret.length > 0 && secret.length <= 4096 && !/\s/.test(secret));
  return <Modal title={modelOnly ? "Add model" : connection ? "Edit provider" : "Add provider"} placement="right" size="wide" showClose onDismiss={onCancel} actions={<><Button variant="ghost" type="button" disabled={busy} onClick={onCancel}>Cancel provider edit</Button><Button form={formId} type="submit" disabled={busy || !valid}>{modelOnly && connection ? "Add models to connection" : connection ? "Save connection" : "Create connection"}</Button></>}>
    <div className="ai-provider-workspace"><form id={formId} className="ai-provider-editor" aria-label={connection ? "Edit provider connection" : "Add provider connection"} onSubmit={(event) => { event.preventDefault(); if (!valid || busy) return; const body = { provider, name: modelOnly && connection ? connection.name : name.trim(), models: finalModels, enabled: modelOnly && connection ? connection.enabled : enabled, ...(secret ? { api_key: secret } : {}) }; setSecret(""); void onSave(connection ? { ...body, expected_revision: connection.revision } : { ...body, api_key: secret }, connection); }}>
    ...
    {initialConnection ? <p>Provider: {providerLabel}. A connection's provider cannot be changed.</p> : <div className="ai-provider-picker"><Field label="Search providers"><Input value={providerSearch} onChange={(e) => setProviderSearch(e.target.value)} placeholder="Find an AI provider" /></Field><div role="group" aria-label="Choose provider">{definitions.filter((d) => `${d.name} ${d.id}`.toLowerCase().includes(providerSearch.toLowerCase())).map((d) => <button type="button" key={d.id} aria-pressed={provider === d.id} onClick={() => switchProvider(d.id)}><span className="ai-provider-monogram" aria-hidden="true">{d.name.slice(0, 1)}</span><span>{d.name}<small>{d.credential_label}</small></span></button>)}</div>{!definitions.filter((d) => `${d.name} ${d.id}`.toLowerCase().includes(providerSearch.toLowerCase())).length && <p>No supported providers match your search.</p>}</div>}
    {modelOnly && <><Field label="Credential connection"><select value={existingID} onChange={(e) => { setExistingID(e.target.value); setSecret(""); setModels(""); }}><option value="">Create a new connection</option>{connections.filter((c) => c.provider === provider && (c.status === "applied" && c.applied_revision === c.revision || c.status === "disabled")).map((c) => <option key={c.id} value={c.id}>{c.name} · {c.status}</option>)}</select></Field><p>Pending or failed connections need their key re-entered through Edit before reuse.</p>{connection && <p>Reuse {connection.name} without re-entering its API key. Existing models are preserved.</p>}</>}
    {!(modelOnly && connection) && <><div className="ai-provider-form-grid"><Field label="Connection name"><Input autoFocus value={name} maxLength={80} onChange={(e) => setName(e.target.value)} placeholder={`Engineering ${providerLabel}`} /></Field><Field label={connection ? "Replacement API key (optional)" : definition?.credential_label ?? "Provider API key"}><Input type="password" autoComplete="new-password" spellCheck={false} value={secret} maxLength={4096} onChange={(e) => setSecret(e.target.value)} placeholder={connection ? "Leave blank to keep the current key" : "Enter your provider key"} /></Field></div>
    <p>Keys are stored securely and never shown again. If a key update fails, enter the key again and save.</p></>}
    <div className="ai-provider-catalog"><div className="ai-provider-catalog-search"><Field label="Search model catalog"><Input value={query} maxLength={100} onChange={(e) => { setQuery(e.target.value); searchSerial.current++; setSearching(false); setCatalog(null); }} placeholder="Search model name" /></Field><Button type="button" disabled={searching || !definition} onClick={() => void search()}>{searching ? "Searching…" : "Search models"}</Button></div>{catalogError && <p role="alert">{catalogError}</p>}{catalog && <><p>{catalog.total} matching models · showing {catalog.items.length ? catalog.offset + 1 : 0}–{catalog.offset + catalog.items.length}</p><div className="ai-provider-catalog-items">{catalog.items.map((m) => <label key={m.id}><input type="checkbox" checked={chosen.includes(m.id)} disabled={!chosen.includes(m.id) && chosen.length >= 32} onChange={(e) => setModels((e.target.checked ? [...chosen, m.id] : chosen.filter((id) => id !== m.id)).join("\n"))} /><span>{m.name}<small>{m.id}</small></span></label>)}</div><div className="ai-provider-form-actions"><Button type="button" disabled={searching || catalog.offset === 0} onClick={() => void search(Math.max(0, catalog.offset - catalog.limit))}>Previous models</Button><Button type="button" disabled={searching || catalog.offset + catalog.limit >= catalog.total || catalog.offset + catalog.limit > 10000} onClick={() => void search(catalog.offset + catalog.limit)}>Next models</Button></div></>}</div>
    <Field label="Exact model IDs (one per line)"><textarea value={models} rows={4} onChange={(e) => setModels(e.target.value)} placeholder={definition?.model_placeholder ?? `${provider}/upstream-model`} /></Field><p>{finalModels.length}/32 models selected. Suggestions do not prove this key can run every model.</p>
