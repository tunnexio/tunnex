...
function ProviderWorkspace({ orgId }: { orgId: string }) {
  ...
  return <section className="ai-provider-workspace" aria-label="Models and endpoints">
    ...
    {loading && !inventory ? <p role="status">Loading provider connections…</p> : !inventory ? <Button onClick={() => { setError(""); void reload(); }}>Retry provider connections</Button> : <>
      ...
      {(editing || view === "add") && <ProviderEditor key={editing === "new" ? "new-credentials" : editing ? `${editing.id}:${editing.revision}` : "new-model"} orgId={orgId} connection={editing === "new" ? undefined : editing ?? undefined} definitions={definitions} testAvailable={inventory.test_available ?? false} foundryAvailable={inventory.foundry_available ?? false} foundryEndpoints={inventory.foundry_endpoints ?? []} sagemakerAvailable={inventory.sagemaker_available ?? false} sagemakerEndpoints={inventory.sagemaker_endpoints ?? []} customAvailable={inventory.custom_available ?? false} endpoints={inventory.custom_endpoints ?? []} connections={inventory.items} modelOnly={!editing} busy={busy} onCancel={() => editing ? setEditing(null) : changeView("models")} onSave={save} />}
      {view !== "add" && <Button disabled={loading || busy} onClick={() => { setError(""); void reload(); }}>Refresh models and credentials</Button>}
    </>}
  </section>;
}
function ProviderEditor({ orgId, connection: initialConnection, definitions, testAvailable, foundryAvailable, foundryEndpoints, sagemakerAvailable, sagemakerEndpoints, customAvailable, endpoints, connections, modelOnly, busy, onSave, onCancel }: { orgId: string; connection?: AIProviderConnection; definitions: Definition[]; testAvailable: boolean; foundryAvailable: boolean; foundryEndpoints: NonNullable<S["AIProviderList"]["foundry_endpoints"]>; sagemakerAvailable: boolean; sagemakerEndpoints: NonNullable<S["AIProviderList"]["sagemaker_endpoints"]>; customAvailable: boolean; endpoints: NonNullable<S["AIProviderList"]["custom_endpoints"]>; connections: AIProviderConnection[]; modelOnly: boolean; busy: boolean; onSave: (body: S["AIProviderCreate"] | S["AIProviderUpdate"], connection?: AIProviderConnection) => Promise<void>; onCancel: () => void }) {
  ...
  const endpointAvailable = provider === "azure_foundry" ? foundryAvailable : provider === "sagemaker" ? sagemakerAvailable : customAvailable;
  ...
  async function search(offset = 0) {
    if (!provider || usesEndpoint && !connection) return;
    const n = ++searchSerial.current;
    setSearching(true); setCatalogError(""); setCatalogOpen(true);
    try {
      const r = await api.GET("/api/v1/organizations/{orgId}/ai-gateway/models", { params: { path: { orgId }, query: { provider, ...(usesEndpoint && connection ? { connection_id: connection.id } : {}), query: query.trim(), limit: 50, offset } } });
  ...
  const foundryFormatValid = provider !== "azure_foundry" || /^https:\/\/[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.(?:openai\.azure\.com|services\.ai\.azure\.com)\/openai$/.test(selectedEndpoint);
  ...
  const valid = (!usesEndpoint || endpointAvailable && endpointApproved && foundryFormatValid) && !!provider && (!!connection || !!definition) && (name.trim().length <= 80) && chosen.length > 0 && finalModels.length <= 32 && chosen.every(validModel) && (connection && !secret || secret.length > 0 && secret.length <= 4096 && !/\s/.test(secret));
  ...
  async function testConnection() {
    ...
    try {
      const r = await api.POST("/api/v1/organizations/{orgId}/ai-gateway/providers/test-connection", { params: { path: { orgId } }, body: { provider, model, api_key: secret, ...(usesEndpoint ? { endpoint_url: selectedEndpoint } : {}) } });
  ...
  const editor = (
    ...
    {usesEndpoint && (connection ? <><Field label="Upstream API Base"><Input readOnly value={connection.endpoint_url ?? ""} /></Field><p>The endpoint cannot be changed for this connection.</p></> : <div className="ai-provider-endpoint-entry">
      ...
      <p>{provider === "azure_foundry" ? "Enter your Azure HTTPS base ending /openai/v1. Both resource.services.ai.azure.com and resource.openai.azure.com are supported; the endpoint must be installation-approved." : provider === "sagemaker" ? "Enter your private LiteLLM bridge URL. AWS region, endpoint and IAM settings stay on that bridge; use its scoped gateway key." : "Enter an OpenAI-compatible API base URL. A trailing /v1 is optional."}</p>
      {!endpointAvailable ? <p role="alert">Installation setup required: approve this endpoint and configure secure egress before testing.</p> : endpoint.trim() && !enteredBase ? <p role="alert">Enter an HTTP or HTTPS base URL without embedded credentials, query parameters or a fragment.</p> : endpoint.trim() && !foundryFormatValid ? <p role="alert">Use an Azure HTTPS resource URL ending /openai/v1, without an api-version query.</p> : endpoint.trim() && !endpointApproved ? <p role="alert">This endpoint is not approved for this provider. Ask your installation administrator to add its URL and network allowlist before testing.</p> : endpointApproved ? <p>Test request: <code>{selectedEndpoint.replace(/\/+$/, "")}/v1/chat/completions</code></p> : null}
    </div>)}
