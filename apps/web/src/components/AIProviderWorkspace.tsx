...
function ProviderWorkspace({ orgId }: { orgId: string }) {
  ...
  async function mutate(call: () => Promise<{ data?: unknown; error?: unknown; response: Response }>, success: string) {
    ...
    try {
      ...
      if (r.error) {
        setError(r.response.status === 409 ? "This connection changed or is referenced by a team policy. Refresh and update those policies before removing models, changing their mode or deleting the connection." : "The operation could not be completed. Check the connection state; an uncertain key update requires entering the API key again.");
...
function ProviderEditor({ orgId, connection: initialConnection, definitions, supportedModes, testAvailable, publicEndpointsAvailable, foundryAvailable, foundryEndpoints, sagemakerAvailable, sagemakerEndpoints, customAvailable, endpoints, connections, modelOnly, busy, onSave, onCancel }: { orgId: string; connection?: AIProviderConnection; definitions: Definition[]; supportedModes: ModelMode[]; testAvailable: boolean; publicEndpointsAvailable: boolean; foundryAvailable: boolean; foundryEndpoints: NonNullable<S["AIProviderList"]["foundry_endpoints"]>; sagemakerAvailable: boolean; sagemakerEndpoints: NonNullable<S["AIProviderList"]["sagemaker_endpoints"]>; customAvailable: boolean; endpoints: NonNullable<S["AIProviderList"]["custom_endpoints"]>; connections: AIProviderConnection[]; modelOnly: boolean; busy: boolean; onSave: (body: S["AIProviderCreate"] | S["AIProviderUpdate"], connection?: AIProviderConnection) => Promise<void>; onCancel: () => void }) {
  ...
  const editor = (
    ...
    {provider === "azure_foundry" && <p>Azure OpenAI v1. Use the exact deployed model name and your Azure API key. Legacy /models, api-version URLs, Anthropic Messages, managed identity and sovereign clouds are not supported here.</p>}
