...
function ProviderWorkspace({ orgId }: { orgId: string }) {
  ...
  return <section className="ai-provider-workspace" aria-label="Models and endpoints">
    ...
    {loading && !inventory ? <p role="status">Loading provider connections…</p> : !inventory ? <Button onClick={() => { setError(""); void reload(); }}>Retry provider connections</Button> : <>
      ...
      {(editing || view === "add") && <ProviderEditor key={editing === "new" ? "new-credentials" : editing ? `${editing.id}:${editing.revision}` : "new-model"} orgId={orgId} connection={editing === "new" ? undefined : editing ?? undefined} definitions={definitions} testAvailable={inventory.test_available ?? false} publicEndpointsAvailable={inventory.public_endpoints_available ?? false} foundryAvailable={inventory.foundry_available ?? false} foundryEndpoints={inventory.foundry_endpoints ?? []} sagemakerAvailable={inventory.sagemaker_available ?? false} sagemakerEndpoints={inventory.sagemaker_endpoints ?? []} customAvailable={inventory.custom_available ?? false} endpoints={inventory.custom_endpoints ?? []} connections={inventory.items} modelOnly={!editing} busy={busy} onCancel={() => editing ? setEditing(null) : changeView("models")} onSave={save} />}
      {view !== "add" && <Button disabled={loading || busy} onClick={() => { setError(""); void reload(); }}>Refresh models and credentials</Button>}
    </>}
  </section>;
}
function ProviderEditor({ orgId, connection: initialConnection, definitions, testAvailable, publicEndpointsAvailable, foundryAvailable, foundryEndpoints, sagemakerAvailable, sagemakerEndpoints, customAvailable, endpoints, connections, modelOnly, busy, onSave, onCancel }: { orgId: string; connection?: AIProviderConnection; definitions: Definition[]; testAvailable: boolean; publicEndpointsAvailable: boolean; foundryAvailable: boolean; foundryEndpoints: NonNullable<S["AIProviderList"]["foundry_endpoints"]>; sagemakerAvailable: boolean; sagemakerEndpoints: NonNullable<S["AIProviderList"]["sagemaker_endpoints"]>; customAvailable: boolean; endpoints: NonNullable<S["AIProviderList"]["custom_endpoints"]>; connections: AIProviderConnection[]; modelOnly: boolean; busy: boolean; onSave: (body: S["AIProviderCreate"] | S["AIProviderUpdate"], connection?: AIProviderConnection) => Promise<void>; onCancel: () => void }) {
  ...
  async function search(offset = 0) {
    ...
    try {
      const r = draftCatalog
        ? await api.POST("/api/v1/organizations/{orgId}/ai-gateway/providers/model-catalog", { params: { path: { orgId } }, body: { provider, api_key: secret, endpoint_url: selectedEndpoint, query: query.trim(), limit: 50, offset } })
        : await api.GET("/api/v1/organizations/{orgId}/ai-gateway/models", { params: { path: { orgId }, query: { provider, ...(usesEndpoint && connection ? { connection_id: connection.id } : {}), query: query.trim(), limit: 50, offset } } });
  ...
  function switchProvider(next: AIProviderConnection["provider"]) {
    ...
    setProvider(next); setManualModel(""); setCatalogOpen(false); setCatalogDismissed(false); setEndpoint(""); setSecret(""); setModels(""); setExistingID(""); setQuery(""); setCatalog(null); setCatalogError(""); setSearching(false); searchSerial.current++;
  }
  ...
  const foundryFormatValid = provider !== "azure_foundry" || /^https:\/\/[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.(?:openai\.azure\.com|services\.ai\.azure\.com|cognitiveservices\.azure\.com)\/openai$/.test(selectedEndpoint);
  ...
  const canSearch = !!definition && (!draftCatalog || endpointReady && secretValid);
  ...
  useEffect(() => { searchSerial.current++; setCatalog(null); setCatalogError(""); setCatalogOpen(false); setSearching(false); }, [provider, searchEndpoint, searchSecret, searchConnection]);
  useEffect(() => {
    if (!canSearch || catalogDismissed) return;
    const timer = window.setTimeout(() => { void search(0); }, 500);
    return () => window.clearTimeout(timer);
  }, [provider, query, canSearch, searchEndpoint, searchSecret, searchConnection, catalogDismissed]);
  ...
  const editor = (
    ...
    {usesEndpoint && (connection ? <><Field label="Upstream API Base"><Input readOnly value={connection.endpoint_url ?? ""} /></Field><p>The endpoint cannot be changed for this connection.</p></> : <div className="ai-provider-endpoint-entry">
      ...
      <p>{provider === "azure_foundry" ? "Enter your Azure HTTPS base ending /openai/v1. Resource hosts under services.ai.azure.com, openai.azure.com and cognitiveservices.azure.com are supported." : provider === "sagemaker" ? "Enter your private LiteLLM bridge URL. AWS region, endpoint and IAM settings stay on that bridge; use its scoped gateway key." : "Enter an OpenAI-compatible API base URL. A trailing /v1 is optional."}</p>
      ...
      {!endpointAvailable ? <p role="alert">Installation setup required: configure secure provider egress before testing.</p> : endpoint.trim() && !enteredBase ? <p role="alert">Enter an HTTP or HTTPS base URL without embedded credentials, query parameters or a fragment.</p> : endpoint.trim() && !foundryFormatValid ? <p role="alert">Use an Azure HTTPS resource URL ending /openai/v1, without an api-version query.</p> : endpoint.trim() && !endpointApproved ? <p role="alert">This endpoint needs configured network access for this provider. Use a public HTTPS endpoint when available, or ask your installation administrator to configure private access.</p> : endpointApproved ? <p>Test request: <code>{selectedEndpoint.replace(/\/+$/, "")}/v1/chat/completions</code></p> : null}
    </div>)}
