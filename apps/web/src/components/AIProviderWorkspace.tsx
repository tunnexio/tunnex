...
function ProviderWorkspace({ orgId }: { orgId: string }) {
  ...
  return <section className="ai-provider-workspace" aria-label="Provider connections">
    ...
    {loading && !inventory ? <p role="status">Loading provider connections…</p> : !inventory ? <Button onClick={() => { setError(""); void reload(); }}>Retry provider connections</Button> : <>
      ...
      <p className="ai-provider-footnote">Catalog/connection checks generate no model tokens. Public catalogs may not validate API keys. Success does not prove model inference access.</p>
      ...
      {editing && <ProviderEditor key={typeof editing === "string" ? "new" : `${editing.id}:${editing.revision}`} orgId={orgId} connection={typeof editing === "string" ? undefined : editing} definitions={definitions} testAvailable={inventory.test_available ?? false} sagemakerAvailable={inventory.sagemaker_available ?? false} sagemakerEndpoints={inventory.sagemaker_endpoints ?? []} customAvailable={inventory.custom_available ?? false} endpoints={inventory.custom_endpoints ?? []} connections={inventory.items} modelOnly={editing === "model"} busy={busy} onCancel={() => setEditing(null)} onSave={save} />}
...
function ProviderEditor({ orgId, connection: initialConnection, definitions, testAvailable, sagemakerAvailable, sagemakerEndpoints, customAvailable, endpoints, connections, modelOnly, busy, onSave, onCancel }: { orgId: string; connection?: AIProviderConnection; definitions: Definition[]; testAvailable: boolean; sagemakerAvailable: boolean; sagemakerEndpoints: NonNullable<S["AIProviderList"]["sagemaker_endpoints"]>; customAvailable: boolean; endpoints: NonNullable<S["AIProviderList"]["custom_endpoints"]>; connections: AIProviderConnection[]; modelOnly: boolean; busy: boolean; onSave: (body: S["AIProviderCreate"] | S["AIProviderUpdate"], connection?: AIProviderConnection) => Promise<void>; onCancel: () => void }) {
  ...
  return <Modal title={modelOnly ? "Add model" : connection ? "Edit provider" : "Add provider"} placement="right" size="wide" showClose onDismiss={onCancel} actions={<><Button variant="ghost" type="button" disabled={busy} onClick={onCancel}>Cancel provider edit</Button><Button form={formId} type="submit" disabled={busy || !canSave}>{modelOnly && connection ? "Add models to connection" : connection ? "Save connection" : "Create connection"}</Button></>}>
    <div className="ai-provider-workspace"><form id={formId} className="ai-provider-editor" aria-label={connection ? "Edit provider connection" : "Add provider connection"} onSubmit={(event) => { event.preventDefault(); if (!canSave || busy || !provider) return; const body = { provider, ...(usesBridge ? { endpoint_url: selectedEndpoint } : {}), name: modelOnly && connection ? connection.name : name.trim(), models: finalModels, enabled: modelOnly && connection ? connection.enabled : enabled, ...(secret ? { api_key: secret } : {}) }; setSecret(""); void onSave(connection ? { ...body, expected_revision: connection.revision } : { ...body, api_key: secret }, connection); }}>
    ...
    {initialConnection ? <p className="ai-provider-fixed-brand"><ProviderLogo provider={initialConnection.provider} />Provider: {providerLabel}. A connection's provider cannot be changed.</p> : <div className="ai-provider-picker"><EntityPicker label="Provider" value={provider ?? ""} placeholder="Select a provider" options={definitions.map((d) => ({ value: d.id, kind: "provider", tag: "", label: d.name, icon: <ProviderLogo provider={d.id} />, unavailable: (d.id === "custom" && !customAvailable || d.id === "sagemaker" && !sagemakerAvailable) ? "Installation setup required" : undefined }))} onSelect={(option) => { const selected = definitions.find((d) => d.id === option.value); if (selected) switchProvider(selected.id); }} /></div>}
    {provider === "sagemaker" && <p>AWS region, endpoint and IAM credentials are managed by your installation’s LiteLLM bridge. Enter its scoped gateway key and configured model alias.</p>}
    ...
    {modelOnly && <><Field label="Credential connection"><select value={existingID} onChange={(e) => { setExistingID(e.target.value); setManualModel(""); setCatalogOpen(false); setSecret(""); if (usesBridge) setModels(""); setCatalog(null); setCatalogError(""); setSearching(false); searchSerial.current++; }}><option value="">Create a new connection</option>{connections.filter((c) => c.provider === provider && (c.status === "applied" && c.applied_revision === c.revision || c.status === "disabled")).map((c) => <option key={c.id} value={c.id}>{c.name} · {c.status}</option>)}</select></Field><p>Pending or failed connections need their key re-entered through Edit before reuse.</p>{connection && <p>Reuse {connection.name} without re-entering its API key. Existing models are preserved.</p>}</>}
    {usesBridge && (connection ? <><Field label="Upstream endpoint"><Input readOnly value={connection.endpoint_url ?? ""} /></Field><p>The endpoint cannot be changed for this connection.</p></> : <Field label="Approved upstream endpoint"><select value={endpoint} onChange={(e) => { setEndpoint(e.target.value); setSecret(""); }}><option value="">Select an approved endpoint</option>{approvedEndpoints.map((e) => <option key={e.url} value={e.url}>{e.name} · {e.url}</option>)}</select></Field>)}
    {!(modelOnly && connection) && <><div className="ai-provider-form-grid"><Field label="Connection name"><Input autoFocus value={name} maxLength={80} onChange={(e) => setName(e.target.value)} placeholder="e.g. Engineering AI" /></Field><Field label={connection ? "Replacement API key (optional)" : provider === "sagemaker" ? "Gateway API key" : "API key"}><Input type="password" autoComplete="new-password" spellCheck={false} value={secret} maxLength={4096} onChange={(e) => setSecret(e.target.value)} placeholder={connection ? "Leave blank to keep the current key" : provider === "sagemaker" ? "Enter your scoped gateway key" : "Enter your provider key"} /></Field></div>
    ...
    {needsTest && <div className="ai-provider-preflight"><Button type="button" disabled={busy || testing || !valid || !testAvailable} onClick={() => void testConnection()}>{testing ? "Testing connection…" : "Test Connect"}</Button><p>A small inference request tests the first selected model only; charges may apply and are excluded from gateway usage totals. Success does not certify the other selected models. Results expire after five minutes.</p>{!testAvailable && <p>Test Connect requires installation setup of the private LiteLLM bridge.</p>}{testStatus === "success" && <p role="status">Test succeeded for {chosen[0]}. You can now save this connection.</p>}{testStatus === "error" && <p role="alert">Connection test failed. Check the model, endpoint and credentials, then retry.</p>}</div>}
  </form></div>
  </Modal>;
}
