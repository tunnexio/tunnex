...
export function AIProviderWorkspace({ orgId, canManage = true }: { orgId: string; canManage?: boolean }) {
  return <ProviderWorkspace key={orgId} orgId={orgId} canManage={canManage} />;
}
function ProviderWorkspace({ orgId, canManage }: { orgId: string; canManage: boolean }) {
...
function ProviderEditor({ orgId, connection: initialConnection, definitions, supportedModes, testAvailable, publicEndpointsAvailable, foundryAvailable, foundryEndpoints, sagemakerAvailable, sagemakerEndpoints, customAvailable, endpoints, connections, modelOnly, busy, onSave, onCancel }: { orgId: string; connection?: AIProviderConnection; definitions: Definition[]; supportedModes: ModelMode[]; testAvailable: boolean; publicEndpointsAvailable: boolean; foundryAvailable: boolean; foundryEndpoints: NonNullable<S["AIProviderList"]["foundry_endpoints"]>; sagemakerAvailable: boolean; sagemakerEndpoints: NonNullable<S["AIProviderList"]["sagemaker_endpoints"]>; customAvailable: boolean; endpoints: NonNullable<S["AIProviderList"]["custom_endpoints"]>; connections: AIProviderConnection[]; modelOnly: boolean; busy: boolean; onSave: (body: S["AIProviderCreate"] | S["AIProviderUpdate"], connection?: AIProviderConnection) => Promise<void>; onCancel: () => void }) {
  ...
  const editor = (
    ...
    {provider === "azure_foundry" && <p>Connect Azure-hosted models, including Llama, DeepSeek, Phi, Mistral and GPT, using your deployment name and Azure API key. This connection uses Foundry's OpenAI-compatible v1 API. Claude requires the separate Anthropic Messages protocol, which this connection does not yet support.</p>}
    ...
    {usesEndpoint && (connection ? <><Field label="Upstream API Base"><Input readOnly value={connection.endpoint_url ?? ""} /></Field><p>The endpoint cannot be changed for this connection.</p></> : <div className="ai-provider-endpoint-entry">
      ...
      <p>{provider === "azure_foundry" ? "Paste your Azure resource URL, /openai/v1 base, or full deployment request URL from Azure. Requests use the resource's /openai/v1 API." : provider === "sagemaker" ? "Enter your private LiteLLM bridge URL. AWS region, endpoint and IAM settings stay on that bridge; use its scoped gateway key." : "Enter an OpenAI-compatible API base URL. A trailing /v1 is optional."}</p>
