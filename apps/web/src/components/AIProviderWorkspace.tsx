...
function ProviderWorkspace({ orgId }: { orgId: string }) {
  ...
  const [editing, setEditing] = useState<AIProviderConnection | "model" | null>(null);
  const [view, setView] = useState<"connections" | "models">("models");
  ...
  return <section className="ai-provider-workspace" aria-label="Models and endpoints">
    <header className="ai-provider-heading"><div><p className="ai-provider-eyebrow">AI GATEWAY / MODELS</p><h2>Models & endpoints</h2><p>Add a model with your provider credentials, then grant your teams access.</p></div><Button disabled={loading || busy || !inventory?.management_available || !definitions.length} onClick={() => { setEditing("model"); setRemoving(null); }}>Add Model</Button></header>
    ...
    {loading && !inventory ? <p role="status">Loading provider connections…</p> : !inventory ? <Button onClick={() => { setError(""); void reload(); }}>Retry provider connections</Button> : <>
      ...
      <Button disabled={loading || busy} onClick={() => { setError(""); void reload(); }}>Refresh models and credentials</Button>
    </>}
  </section>;
}
