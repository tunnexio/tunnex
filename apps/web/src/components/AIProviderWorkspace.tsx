...
function ProviderEditor({ orgId, connection: initialConnection, definitions, testAvailable, sagemakerAvailable, sagemakerEndpoints, customAvailable, endpoints, connections, modelOnly, busy, onSave, onCancel }: { orgId: string; connection?: AIProviderConnection; definitions: Definition[]; testAvailable: boolean; sagemakerAvailable: boolean; sagemakerEndpoints: NonNullable<S["AIProviderList"]["sagemaker_endpoints"]>; customAvailable: boolean; endpoints: NonNullable<S["AIProviderList"]["custom_endpoints"]>; connections: AIProviderConnection[]; modelOnly: boolean; busy: boolean; onSave: (body: S["AIProviderCreate"] | S["AIProviderUpdate"], connection?: AIProviderConnection) => Promise<void>; onCancel: () => void }) {
  ...
  const valid = (!usesBridge || bridgeAvailable && endpointApproved) && !!provider && (!!connection || !!definition) && (connection && modelOnly || name.trim().length > 0 && name.trim().length <= 80) && chosen.length > 0 && finalModels.length <= 32 && chosen.every(validModel) && (connection && !secret || secret.length > 0 && secret.length <= 4096 && !/\s/.test(secret));
  ...
  return <Modal title={modelOnly ? "Add model" : connection ? "Edit provider" : "Add provider"} placement="right" size="wide" showClose onDismiss={onCancel} actions={<><Button variant="ghost" type="button" disabled={busy} onClick={onCancel}>Cancel provider edit</Button>{needsTest && <Button type="button" disabled={busy || testing || !valid || !testAvailable} onClick={() => void testConnection()}>{testing ? "Testing connection…" : "Test Connect"}</Button>}<Button form={formId} type="submit" disabled={busy || !canSave}>{modelOnly && connection ? "Add models to connection" : connection ? "Save connection" : "Create connection"}</Button></>}>
    ...
    {provider && !usesBridge && <p>{providerLabel} uses its standard API endpoint. For a different OpenAI-compatible API URL, choose Custom provider.</p>}
