...
export function agentBootstrapCommand(token: string, release: BootstrapRelease, apiURL = globalThis.location?.origin ?? ""): string {
  ...
  const script = [
    ...
    `curl -fsSL "$manifest_url" -o /dev/null; curl -fsSL "\${manifest_url%release.json}$runtime_name" -o "$d/runtime"; actual_digest=$(sha256sum "$d/runtime" | awk '{print $1}'); [ "$actual_digest" = "$runtime_digest" ] || { echo 'runtime byte digest refused' >&2; exit 1; }; curl -fsSL "\${manifest_url%release.json}$unit_name" -o "$d/unit"; actual_unit_digest=$(sha256sum "$d/unit" | awk '{print $1}'); [ "$actual_unit_digest" = "$unit_digest" ] || { echo 'runtime unit refused' >&2; exit 1; }`,
