...
function PolicyTemplatesWorkspace({ orgId }: { orgId: string }) {
  ...
  return <div className="space-y-4">
    ...
    {dialog === "version" && <Modal title="Create immutable version" onDismiss={() => setDialog(null)} actions={<><Button variant="ghost" onClick={() => setDialog(null)}>Cancel</Button><Button disabled={busy || !resourceId} onClick={() => void createVersion()}>Create version</Button></>}><p className="mb-3 text-cell text-ink-tertiary">This creates a new immutable version; it does not change an existing assignment.</p>{resources.length === 0 && <p role="status" className="mb-3">No destination resources yet. <Link className="text-accent-400" to="/access/resources">Create a resource</Link>, then return here to add it to this template.</p>}<Field label="Destination resource"><Select value={resourceId} onChange={(event) => setResourceId(event.target.value)}><option value="">Select resource</option>{resources.map((resource) => <option key={resource.id} value={resource.id}>{resource.name}</option>)}</Select></Field></Modal>}
