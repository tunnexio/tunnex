...
export default function AccessResources() {
  ...
  return <div className="space-y-5">{header}<ErrorText>{error}</ErrorText><div className="max-w-md"><Input aria-label="Search resources" value={query} placeholder="Search resource name, CIDR, or protocol" onChange={(event) => setQuery(event.target.value)} /></div><Card><DataTable caption="Resources inventory" rows={filteredResources} rowKey={(resource) => resource.id} failed={false} filterable={false} pageSize={25} empty={<EmptyState>{resources.length === 0 ? "No resources yet. Define named destination ranges so policy rules stay readable and reusable." : "No resources match this search."}</EmptyState>} columns={[{ key: "name", header: "Resource", cell: (resource) => <button className="font-medium text-ink-heading hover:underline" onClick={() => openEdit(resource)}>{resource.name}</button> }, { key: "label", header: "Description", cell: (resource) => resource.label || "—" }, { key: "cidr", header: "CIDR", cell: (resource) => resource.cidr }, { key: "protocol", header: "Protocol", cell: (resource) => resource.protocol === "any" ? "Any" : resource.protocol.toUpperCase() }, { key: "ports", header: "Ports", cell: (resource) => resource.protocol === "any" || resource.port_low == null ? "All" : resource.port_high == null || resource.port_high === resource.port_low ? String(resource.port_low) : `${resource.port_low}–${resource.port_high}` }, { key: "actions", header: "", cell: (resource) => <div className="flex gap-2"><Button size="sm" variant="ghost" onClick={() => openEdit(resource)}>Edit</Button><Button size="sm" variant="danger" onClick={() => { setSelected(resource); setDialog("delete"); }}>Delete</Button></div> }]} /></Card>
    <FQDNResources orgId={org.id} role={role} />
...
function FQDNResources({ orgId, role }: { orgId: string; role: Member["role"] | undefined }) {
  ...
  const loadContexts = useCallback(async () => {
    ...
    if (!sitesResult.ok || !nodesResult.ok) { setError(!sitesResult.ok ? sitesResult.error : nodesResult.error); return; }
  ...
  async function save() {
    ...
    const result = selected
      ? await api.PATCH("/api/v1/organizations/{orgId}/fqdn-resources/{resourceId}", { params: { path: { orgId, resourceId: selected.id } }, body })
      : await api.POST("/api/v1/organizations/{orgId}/fqdn-resources", { params: { path: { orgId } }, body });
    setBusy(false);
    if (result.error) { setError(apiErrorMessage(result.error, "Could not save the FQDN resource.")); return; }
  ...
  async function remove() {
    ...
    setBusy(true); const result = await api.DELETE("/api/v1/organizations/{orgId}/fqdn-resources/{resourceId}", { params: { path: { orgId, resourceId: selected.id } } }); setBusy(false);
    if (result.error) { setError(apiErrorMessage(result.error, "Could not delete the FQDN resource.")); return; }
    setDialog(null); setSelected(null); await reload();
  }
  if (!canView) return <Card><div className="space-y-2" aria-labelledby="fqdn-resources-heading"><h2 id="fqdn-resources-heading" className="text-lg font-semibold text-ink-heading">FQDN resources</h2><p role="alert" className="text-cell text-ink-tertiary">You do not have permission to view FQDN resources.</p></div></Card>;
  ...
  return <Card><section aria-labelledby="fqdn-resources-heading" className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 id="fqdn-resources-heading" className="text-lg font-semibold text-ink-heading">FQDN resources</h2><p className="text-cell text-ink-tertiary">Resolver-backed exact hostnames. DNS answers and impacted-rule identities are not exposed by this contract.</p></div>{canManage && <Button onClick={() => openForm()}>Create FQDN resource</Button>}</div>
    {settingError ? <p role="alert" className="text-cell text-danger">FQDN enforcement setting is unavailable: {settingError}</p> : setting === null ? <p role="status" className="text-cell text-ink-tertiary">Loading FQDN enforcement setting…</p> : <p role="status" className="rounded-md border border-border px-3 py-2 text-cell text-ink-tertiary">Enforcement opt-in: <strong className="text-ink-heading">{setting.enabled ? "enabled" : "not enabled"}</strong>. {setting.enabled ? "This permits compilation when a resource is ready." : "Resources can remain drafts; no FQDN resource compiles until an authorized administrator enables it."}</p>}
    <ErrorText>{error}</ErrorText>
    {resources === null ? <Loading label="Loading FQDN resources…" /> : <><div className="max-w-md"><Input aria-label="Search FQDN resources" value={query} placeholder="Search name, hostname, or state" onChange={(event) => setQuery(event.target.value)} /></div><DataTable caption="FQDN resources inventory" rows={filtered} rowKey={(resource) => resource.id} failed={false} filterable={false} pageSize={25} empty={<EmptyState>{resources.length === 0 ? "No FQDN resources yet. Create an exact hostname or save an unbound draft." : "No FQDN resources match this search."}</EmptyState>} columns={[{ key: "name", header: "Resource", cell: (resource) => <button className="font-medium text-ink-heading hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent-400" onClick={() => setSelected(resource)}>{resource.name}</button> }, { key: "fqdn", header: "Hostname", cell: (resource) => resource.fqdn }, { key: "state", header: "State", cell: (resource) => <StateBadge state={resource.state} /> }, { key: "context", header: "Resolver context", cell: (resource) => resource.resolver_context ? `${resource.resolver_context.site_name} / ${resource.resolver_context.gateway_name}` : "Unbound draft" }, { key: "answers", header: "Answers", cell: (resource) => resource.state === "healthy" ? String(resource.answer_count) : "Not available" }, { key: "actions", header: "", cell: (resource) => canManage ? <div className="flex gap-2"><Button size="sm" variant="ghost" onClick={() => openForm(resource)}>Edit</Button><Button size="sm" variant="danger" onClick={() => void openDelete(resource)}>Delete</Button></div> : "Read-only" }]} /></>}
    {selected && !dialog && <FQDNDetail resource={selected} onDismiss={() => setSelected(null)} onEdit={canManage ? () => openForm(selected) : undefined} />}
    {(dialog === "create" || dialog === "edit") && <Modal title={dialog === "edit" ? "Edit FQDN resource" : "Create FQDN resource"} onDismiss={() => setDialog(null)} actions={<><Button variant="ghost" onClick={() => setDialog(null)}>Cancel</Button><Button disabled={busy || !name.trim() || !fqdn.trim() || !portsValid || Boolean(siteId && (!gatewayId || !selectedGatewayValid))} onClick={() => void save()}>{dialog === "edit" ? "Save FQDN resource" : "Create FQDN resource"}</Button></>}><div className="space-y-3"><p className="text-cell text-ink-tertiary">Use one exact hostname. The server normalizes and validates it; a blank resolver context saves a deny-by-default draft.</p><Field label="Name"><Input value={name} autoFocus onChange={(event) => setName(event.target.value)} /></Field><Field label="Exact hostname"><Input value={fqdn} placeholder="orders.internal.example.com" onChange={(event) => setFqdn(event.target.value)} /></Field><Field label="Description (optional)"><Input value={label} onChange={(event) => setLabel(event.target.value)} /></Field><Field label="Protocol"><Select value={protocol} onChange={(event) => { const next = event.target.value as "any" | "tcp" | "udp"; setProtocol(next); if (next === "any") { setPortScope("all"); setPortLow(""); setPortHigh(""); } }}><option value="any">Any protocol</option><option value="tcp">TCP</option><option value="udp">UDP</option></Select></Field>{protocol !== "any" && <><Field label="Port scope"><Select value={portScope} onChange={(event) => setPortScope(event.target.value as "all" | "single" | "range")}><option value="all">All ports</option><option value="single">Single port</option><option value="range">Port range</option></Select></Field>{portScope !== "all" && <div className="grid grid-cols-2 gap-3"><Field label="Port"><Input inputMode="numeric" value={portLow} onChange={(event) => setPortLow(event.target.value)} /></Field>{portScope === "range" && <Field label="Through"><Input inputMode="numeric" value={portHigh} onChange={(event) => setPortHigh(event.target.value)} /></Field>}</div>}</>}<p className="text-xs text-ink-tertiary">Scope: {scope}</p>{!portsValid && <ErrorText>Use whole ports from 1 to 65535; a range must end at or above its starting port.</ErrorText>}<Field label="Resolver site (optional)"><Select value={siteId} onChange={(event) => { setSiteId(event.target.value); setGatewayId(""); }}><option value="">Unbound draft</option>{(contexts?.sites ?? []).map((site) => <option key={site.id} value={site.id}>{site.name}</option>)}</Select></Field>{siteId && <Field label="Gateway"><Select value={gatewayId} onChange={(event) => setGatewayId(event.target.value)}><option value="">Select a gateway</option>{gateways.map((node) => <option key={node.id} value={node.id}>{node.name}</option>)}</Select>{contexts === null ? <p role="status" className="mt-1 text-xs text-ink-tertiary">Loading selected Site/Gateway contexts…</p> : gateways.length === 0 ? <p role="alert" className="mt-1 text-xs text-danger">No active gateway is bound to this site.</p> : null}</Field>}</div></Modal>}
    {dialog === "delete" && selected && <Modal title="Delete FQDN resource?" danger onDismiss={() => setDialog(null)} actions={<><Button variant="ghost" onClick={() => setDialog(null)}>Cancel</Button><Button variant="danger" disabled={busy || !impact || impact.referencing_rule_count > 0 || impact.generation_withdrawal_required} onClick={() => void remove()}>Delete FQDN resource</Button></>}><p className="text-cell text-ink-tertiary">{impactError ? `Server impact could not be loaded: ${impactError}` : !impact ? "Loading server-computed deletion impact…" : <>Server impact: {impact.referencing_rule_count} referencing {impact.referencing_rule_count === 1 ? "rule" : "rules"}; {impact.generation_withdrawal_required ? "a live generation must be withdrawn." : "no live generation needs withdrawal."} {impact.referencing_rule_count > 0 || impact.generation_withdrawal_required ? "Deletion is unavailable until the server-reported impact is cleared." : "If deletion succeeds, recovery requires recreating this resource; immutable generation history may still prevent deletion."}</>}</p></Modal>}
  </section></Card>;
}
...
function FQDNDetail({ resource, onDismiss, onEdit }: { resource: FQDNResource; onDismiss: () => void; onEdit?: () => void }) {
  return <Modal title={resource.name} onDismiss={onDismiss} actions={<><Button variant="ghost" onClick={onDismiss}>Close</Button>{onEdit && <Button onClick={onEdit}>Edit</Button>}</>}><div className="space-y-3 text-cell text-ink-tertiary"><p><strong className="text-ink-heading">{resource.fqdn}</strong> · {resource.protocol === "any" ? "Any protocol, all ports" : `${resource.protocol.toUpperCase()}, all ports`}</p><StateBadge state={resource.state} /><dl className="grid grid-cols-1 gap-2 sm:grid-cols-2"><div><dt>Resolver context</dt><dd className="text-ink-heading">{resource.resolver_context ? `${resource.resolver_context.site_name} / ${resource.resolver_context.gateway_name}` : "Unbound draft"}</dd></div><div><dt>Generation</dt><dd className="text-ink-heading">{resource.generation ?? "Not active"}</dd></div><div><dt>Answer count</dt><dd className="text-ink-heading">{resource.state === "healthy" ? resource.answer_count : "Not available"}</dd></div><div><dt>Effective TTL</dt><dd className="text-ink-heading">{resource.effective_ttl_seconds == null ? "Not available" : `${resource.effective_ttl_seconds} seconds`}</dd></div><div><dt>Last refresh</dt><dd className="text-ink-heading">{resource.refreshed_at ?? "Not available"}</dd></div><div><dt>Last good</dt><dd className="text-ink-heading">{resource.last_good_at ?? "Not available"}</dd></div></dl><p>Current answer addresses, impacted rule identities, audit links, and diagnostics are not available in the server projection.</p></div></Modal>;
}
