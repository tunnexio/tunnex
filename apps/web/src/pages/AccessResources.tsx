...
function FQDNResources({ orgId, role }: { orgId: string; role: Member["role"] | undefined }) {
  ...
  const loadContexts = useCallback(async () => {
    ...
    if (!sitesResult.ok || !nodesResult.ok) { setContextsError(!sitesResult.ok ? sitesResult.error : nodesResult.error); return; }
  ...
  return <Card><section aria-labelledby="fqdn-resources-heading" className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 id="fqdn-resources-heading" className="text-lg font-semibold text-ink-heading">FQDN resources</h2><p className="text-cell text-ink-tertiary">Resolver-backed exact hostnames. A hostname is not an IP ownership boundary: hostnames sharing an IP and protocol/port cannot be distinguished after resolution. DNS answers and impacted-rule identities are not exposed by this contract.</p></div>{canManage ? <Button onClick={() => openForm()}>Create FQDN resource</Button> : <p className="text-cell text-ink-tertiary">Create, edit, delete, and enforcement controls require <code>fqdn_resource:manage</code> (currently owners and admins).</p>}</div>
...
function FQDNDetail({ resource, onDismiss, onEdit }: { resource: FQDNResource; onDismiss: () => void; onEdit?: () => void }) {
  return <Modal title={resource.name} onDismiss={onDismiss} actions={<><Button variant="ghost" onClick={onDismiss}>Close</Button>{onEdit && <Button onClick={onEdit}>Edit</Button>}</>}><div className="space-y-3 text-cell text-ink-tertiary"><p><strong className="text-ink-heading">{resource.fqdn}</strong> · {fqdnPortScope(resource)}</p><StateBadge state={resource.state} /><dl className="grid grid-cols-1 gap-2 sm:grid-cols-2"><div><dt>Resolver authority</dt><dd className="text-ink-heading">{resource.resolver_context ? `${resource.resolver_context.site_name} / ${resource.resolver_context.gateway_name}` : "Unbound draft — cannot compile or authorize traffic"}</dd></div><div><dt>Generation</dt><dd className="text-ink-heading">{resource.generation ?? "Not active"}</dd></div><div><dt>Answer count</dt><dd className="text-ink-heading">{resource.state === "healthy" ? resource.answer_count : "Not available"}</dd></div><div><dt>Effective TTL</dt><dd className="text-ink-heading">{resource.effective_ttl_seconds == null ? "Not available" : `${resource.effective_ttl_seconds} seconds`}</dd></div><div><dt>Last refresh</dt><dd className="text-ink-heading">{resource.refreshed_at ?? "Not available"}</dd></div><div><dt>Last good</dt><dd className="text-ink-heading">{resource.last_good_at ?? "Not available"}</dd></div></dl><p>Exact-host selection does not establish exclusive ownership of a shared IP: hostnames using the same IP and protocol/port cannot be differentiated after resolution. Current answer addresses, impacted rule identities, audit links, and diagnostics are not available in the server projection.</p></div></Modal>;
}
