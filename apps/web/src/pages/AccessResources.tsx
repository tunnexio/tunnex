...
function FQDNResources({ orgId, role, createToken }: { orgId: string; role: Member["role"] | undefined; createToken: number }) {
  ...
  return <Card><section aria-labelledby="fqdn-resources-heading" className="space-y-4"><div><h2 id="fqdn-resources-heading" className="text-lg font-semibold text-ink-heading">FQDN resources</h2><p className="text-cell text-ink-tertiary">Exact hostnames resolved through an active Private DNS Resolver. Hostnames sharing an IP and protocol/port cannot be distinguished after resolution.</p>{!canManage && <p className="mt-2 text-cell text-ink-tertiary">You have read-only access to FQDN resources.</p>}</div>
    <p className="rounded-md border border-border px-3 py-2 text-cell text-ink-tertiary">New resources inherit the matching resolver profile for their selected Site and Gateway; unmatched names fail closed.</p>
