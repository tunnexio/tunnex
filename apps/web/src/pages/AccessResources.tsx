...
export default function AccessResources() {
  ...
  const header = <><PageHeader title="Resources" subtitle="Define the destinations your policies can reach." actions={authorized === true ? <div className="flex flex-wrap items-center gap-2"><a className="inline-flex min-h-11 items-center rounded-md border border-white/10 px-4 py-2 text-sm font-medium text-slate-200 hover:bg-white/5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent-400" href="/access/resources?type=fqdn#private-dns-heading">Private DNS resolvers</a><Button onClick={() => setDialog("choose")}>Create resource</Button></div> : undefined} /><AccessTabRail /></>;
  const resourceTypeTabs = <nav aria-label="Resource types" className="resource-type-switch"><div>{(["cidr", "fqdn"] as const).map((resourceType) => <button key={resourceType} type="button" aria-current={type === resourceType ? "page" : undefined} className={`relative -mb-px min-h-10 px-0.5 py-2 text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white/35 ${type === resourceType ? "text-white after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-white" : "text-ink-tertiary hover:text-ink-heading"}`} onClick={() => updateIndex({ type: resourceType })}>{resourceType === "cidr" ? "CIDR" : "FQDN"}</button>)}</div></nav>;
  if (membershipError) return <div className="network-management resources-workspace space-y-5">{header}{resourceTypeTabs}<Card><LoadRetry error={`Could not check resource permissions: ${membershipError}`} onRetry={() => setMembershipAttempt((attempt) => attempt + 1)} /></Card></div>;
  if (!org || authorized === null) return <div className="network-management resources-workspace space-y-5">{header}{resourceTypeTabs}<Card><Loading label="Checking resource permissions…" /></Card></div>;
  ...
  if (type === "fqdn") return <div className="network-management resources-workspace space-y-5">{header}{resourceTypeTabs}<FQDNResources key={org.id} orgId={org.id} role={role} createToken={fqdnCreateToken} /><details className="resource-dns-settings" open={typeof window !== "undefined" && ["#private-dns-heading", "#fqdn-enforcement-heading"].includes(window.location.hash)}><summary>DNS configuration <span>Enforcement and private resolvers</span></summary><div className="resource-dns-settings-body"><FQDNEnforcementSetting key={"setting-" + org.id} orgId={org.id} role={role} /><PrivateDNSResolvers key={"resolver-" + org.id} orgId={org.id} role={role} /></div></details>{createChooser}</div>;
  if (!authorized) return <div className="network-management resources-workspace space-y-5">{header}{resourceTypeTabs}{indexControls}<Card><p role="alert" className="text-cell text-ink-tertiary">You do not have permission to manage CIDR resources.</p><ErrorText>{error}</ErrorText></Card></div>;
  if (!resources) return <div className="network-management resources-workspace space-y-5">{header}{resourceTypeTabs}{indexControls}<Card><Loading label="Loading resources…" /><ErrorText>{error}</ErrorText></Card></div>;
  ...
  return <div className="network-management resources-workspace space-y-5">{header}{resourceTypeTabs}<ErrorText>{error}</ErrorText><Card><div className="resource-toolbar">{indexControls}<Select width="auto" aria-label="Sort CIDR resources" className="sm:w-40" value={cidrSort} onChange={(event) => updateCidrQuery({ sort: event.target.value })}><option value="name">Name</option><option value="cidr">CIDR</option><option value="protocol">Protocol</option></Select><Select width="auto" aria-label="CIDR sort direction" className="sm:w-36" value={cidrDir} onChange={(event) => updateCidrQuery({ dir: event.target.value })}><option value="asc">Ascending</option><option value="desc">Descending</option></Select></div><div className="hidden sm:block"><DataTable caption="Resources inventory" rows={filteredResources} rowKey={(resource) => resource.id} failed={false} filterable={false} pageSize={25} empty={<EmptyState>{resources.length === 0 ? "No resources yet. Define named destination ranges so policy rules stay readable and reusable." : "No resources match this search."}</EmptyState>} columns={[{ key: "name", header: "Resource", cell: (resource) => <div className="resource-name"><button className="font-medium text-ink-heading hover:underline" onClick={() => openEdit(resource)}>{resource.name}</button>{resource.label && <small>{resource.label}</small>}</div> }, { key: "cidr", header: "CIDR", cell: (resource) => resource.cidr }, { key: "ports", header: "Access scope", cell: (resource) => resourcePortScope(resource) }, { key: "actions", header: "Actions", cell: (resource) => <div className="flex flex-wrap gap-2"><Button size="sm" variant="ghost" aria-label={`Edit ${resource.name}`} onClick={() => openEdit(resource)}>Edit</Button><Button size="sm" variant="danger" aria-label={`Delete ${resource.name}`} onClick={() => { setSelected(resource); setDialog("delete"); }}>Delete</Button></div> }]} /></div><div className="space-y-3 sm:hidden" aria-label="CIDR resource summaries">{filteredResources.map((resource) => <section key={resource.id} className="rounded-md border border-line p-3"><h3 className="font-medium text-ink-heading">{resource.name}</h3><dl className="mt-2 grid grid-cols-2 gap-2 text-sm"><div><dt className="text-ink-tertiary">CIDR</dt><dd>{resource.cidr}</dd></div><div><dt className="text-ink-tertiary">Scope</dt><dd>{resourcePortScope(resource)}</dd></div></dl><div className="mt-3 flex gap-2"><Button size="sm" variant="ghost" aria-label={`Edit ${resource.name}`} onClick={() => openEdit(resource)}>Edit {resource.name}</Button><Button size="sm" variant="danger" aria-label={`Delete ${resource.name}`} onClick={() => { setSelected(resource); setDialog("delete"); }}>Delete {resource.name}</Button></div></section>)}{filteredResources.length === 0 && <EmptyState>{resources.length === 0 ? "No resources yet. Define named destination ranges so policy rules stay readable and reusable." : "No resources match this search."}</EmptyState>}</div></Card>
    {createChooser}
