...
function FQDNResources({ orgId, role, createToken }: { orgId: string; role: Member["role"] | undefined; createToken: number }) {
  ...
  async function save(bindNow: boolean) {
    if (!name.trim() || !fqdn.trim() || !portsValid || (bindNow && (!siteId || !gatewayId || !selectedGatewayValid))) return;
    // There is no bind operation in the generated contract. "Bind now" is only
    // a save with an explicit server-owned resolver context; it promises neither
    // a successful resolution nor authorization.
    const body = { name: name.trim(), fqdn: fqdn.trim(), label: label.trim() || null, protocol, ...(protocol === "any" || portScope === "all" ? { port_low: null, port_high: null } : portScope === "single" ? { port_low: parsedLow!, port_high: parsedLow! } : { port_low: parsedLow!, port_high: parsedHigh! }), resolver_context: bindNow ? { site_id: siteId, gateway_id: gatewayId } : null };
    const ok = selected
