...
export default function AccessResources() {
  ...
  if (!authorized) return <div className="space-y-5">{header}<Card><p role="alert" className="text-cell text-ink-tertiary">You do not have permission to manage CIDR resources.</p><ErrorText>{error}</ErrorText></Card><FQDNResources orgId={org.id} role={role} /></div>;
