...
export default function AccessResources() {
  ...
  const type = ["all", "cidr", "fqdn"].includes(searchParams.get("type") ?? "") ? searchParams.get("type")! : "all";
  ...
  if (!authorized) return <div className="space-y-5">{header}<Card><p role="alert" className="text-cell text-ink-tertiary">You do not have permission to manage CIDR resources.</p><ErrorText>{error}</ErrorText></Card><FQDNResources key={org.id} orgId={org.id} role={role} createToken={0} /></div>;
  ...
  const filteredResources = type === "fqdn" ? [] : resources.filter((resource) => `${resource.name} ${resource.cidr} ${resource.protocol}`.toLowerCase().includes(query.toLowerCase()));
  return <div className="space-y-5">{header}<ErrorText>{error}</ErrorText><div className="grid max-w-2xl gap-2 sm:grid-cols-2"><Input aria-label="Search resources" value={query} placeholder="Search resources" onChange={(event) => updateIndex({ q: event.target.value })} /><Select aria-label="Resource type" value={type} onChange={(event) => updateIndex({ type: event.target.value })}><option value="all">All resource types</option><option value="cidr">CIDR</option><option value="fqdn">FQDN</option></Select></div><Card><DataTable caption="Resources inventory" rows={filteredResources} rowKey={(resource) => resource.id} failed={false} filterable={false} pageSize={25} empty={<EmptyState>{type === "fqdn" ? "FQDN resources are listed below." : resources.length === 0 ? "No resources yet. Define named destination ranges so policy rules stay readable and reusable." : "No resources match this search."}</EmptyState>} columns={[{ key: "name", header: "Resource", cell: (resource) => <button className="font-medium text-ink-heading hover:underline" onClick={() => openEdit(resource)}>{resource.name}</button> }, { key: "label", header: "Description", cell: (resource) => resource.label || "—" }, { key: "cidr", header: "CIDR", cell: (resource) => resource.cidr }, { key: "protocol", header: "Protocol", cell: (resource) => resource.protocol === "any" ? "Any" : resource.protocol.toUpperCase() }, { key: "ports", header: "Ports", cell: (resource) => resource.protocol === "any" || resource.port_low == null ? "All" : resource.port_high == null || resource.port_high === resource.port_low ? String(resource.port_low) : `${resource.port_low}–${resource.port_high}` }, { key: "actions", header: "Actions", cell: (resource) => <div className="flex flex-wrap gap-2"><Button size="sm" variant="ghost" aria-label={`Edit ${resource.name}`} onClick={() => openEdit(resource)}>Edit</Button><Button size="sm" variant="danger" aria-label={`Delete ${resource.name}`} onClick={() => { setSelected(resource); setDialog("delete"); }}>Delete</Button></div> }]} /></Card>
    <FQDNResources key={org.id} orgId={org.id} role={role} createToken={fqdnCreateToken} />
    {dialog === "choose" && <Modal title="Create resource" onDismiss={() => setDialog(null)} actions={<Button variant="ghost" onClick={() => setDialog(null)}>Cancel</Button>}><p className="mb-4 text-cell text-ink-tertiary">Choose the destination shape. CIDR is a static network range; FQDN is one exact hostname resolved only by its selected server-owned authority.</p><div className="grid gap-3 sm:grid-cols-2"><Button onClick={() => { setDialog(null); openCreate(); }}>Create CIDR resource</Button><Button onClick={() => { setDialog(null); setFqdnCreateToken((token) => token + 1); }}>Create FQDN resource</Button></div></Modal>}
...
export function FQDNResourceDetail() {
  ...
  if (!org || resources === null) return <div className="space-y-5"><PageHeader title="FQDN resource" subtitle="Authoritative resolver-backed destination." /><AccessTabRail /><Card>{error ? <LoadRetry error={`Could not load this FQDN resource: ${error}`} onRetry={() => void reload()} /> : <Loading label="Loading FQDN resource…" />}</Card></div>;
