...
export default function AccessResources() {
  ...
  const indexControls = <div className="flex flex-wrap items-center gap-2">{type === "cidr" && <Input aria-label="Search resources" className="min-w-[14rem] flex-1 sm:max-w-md" value={query} placeholder="Search resources" onChange={(event) => updateIndex({ q: event.target.value })} />}<Select aria-label="Resource type" className="w-full sm:w-36" value={type} onChange={(event) => updateIndex({ type: event.target.value })}><option value="cidr">CIDR</option><option value="fqdn">FQDN</option></Select></div>;
  ...
  if (type === "fqdn") return <div className="space-y-5">{header}{indexControls}<PrivateDNSResolvers key={"resolver-" + org.id} orgId={org.id} role={role} /><FQDNResources key={org.id} orgId={org.id} role={role} createToken={fqdnCreateToken} />{createChooser}</div>;
