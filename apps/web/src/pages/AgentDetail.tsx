...
function AgentDetailWorkspace({ fixture, agentIdOverride }: { fixture?: AgentDetailFixture; agentIdOverride?: string }) {
  ...
  async function updateProfile(body: Record<string, unknown>, failure: string) {
    ...
    setBusy(false);
    if (result.error || !result.data) { setMutationError(failure); return; }
  ...
  return <div className="space-y-5">
    ...
    <AgentsTabRail />
    <div role="tablist" aria-label="Agent workspace" className="flex gap-1 overflow-x-auto border-b border-white/10">
      {tabs.map((item) => <button key={item} type="button" role="tab" aria-selected={tab === item} className={`min-h-10 whitespace-nowrap border-b-2 px-3 text-sm ${tab === item ? "border-accent-400 text-ink-heading" : "border-transparent text-ink-tertiary hover:text-ink-heading"}`} onClick={() => switchTab(item)}>{tabLabel[item]}</button>)}
    </div>
    {tab === "overview" && <ReadState state={profile}>{(data) => <div className="grid gap-4 lg:grid-cols-2"><Panel title="Identity"><dl className="grid grid-cols-2 gap-3 text-cell"><dt className="text-ink-tertiary">Lifecycle</dt><dd>{data.status}</dd><dt className="text-ink-tertiary">Owner</dt><dd>{data.owner_email || "Unassigned"}</dd><dt className="text-ink-tertiary">Managing group</dt><dd>{data.managing_group_name || "None"}</dd><dt className="text-ink-tertiary">Last handshake</dt><dd>{age(data.last_handshake_at)}</dd></dl></Panel><Panel title="Status evidence"><p className="text-cell text-ink-tertiary">Lifecycle is the server-owned agent profile state. Handshake freshness is reported separately and is not inferred as an incident.</p><p className="mt-2 text-xs text-ink-tertiary">Source: agent profile projection. Handshake freshness: {age(data.last_handshake_at)}.</p></Panel>{data.permissions.manage && <Panel title="Profile and lifecycle"><AgentProfileEditor key={`${data.device_id}:${data.status}:${data.environment}:${data.runtime}:${JSON.stringify(data.labels)}`} value={{ environment: data.environment, runtime: data.runtime, labels: data.labels, status: data.status as AgentProfileStatus }} canManageLifecycle={data.permissions.manage} disabled={busy} onSaveMetadata={(value: AgentProfileEditorValue) => void updateProfile(value, "Could not save agent metadata.")} onLifecycleChange={(status) => void updateProfile({ status }, "Could not change the agent lifecycle.")} /></Panel>}{data.permissions.assign && <Panel title="Ownership"><AgentAssignment key={`${data.device_id}:${data.owner_id}:${data.managing_group_id ?? ""}`} profile={data} members={members} groups={groups} busy={busy} onSave={(body) => void updateProfile(body, "Could not save the agent assignment.")} /></Panel>}{data.permissions.revoke && <Panel title="Remove agent"><p className="text-cell text-ink-tertiary">Removal revokes the tunnel credential first, then removes the roster record. Saved access-policy evidence is retained for review.</p><Button className="mt-3" size="sm" variant="danger" disabled={busy} onClick={() => setRemoveOpen(true)}>Remove agent</Button></Panel>}</div>}</ReadState>}
