...
export default function AgentsMCP() {
  ...
  return <div className="network-management agents-workspace space-y-5">
    ...
    <Card><h2 className="font-semibold">Connect a server, then allow its tools</h2><p className="mt-2 text-sm text-ink-tertiary">Create a profile and assign it to an agent group. Connect an agent, complete OAuth if required, then review discovered tools and allow only the tools it needs. Adding a profile does not grant tool access.</p><div className="mt-3 flex flex-wrap gap-4 text-sm"><Link className="underline" to="/agents">Open agents and tool permissions</Link></div></Card>
    ...
    {org && selectedProfile && activeAssignment?.profile_id === selectedProfile.id && members.kind === "ready" && <MCPProfileTools key={`${org.id}/${selectedProfile.id}/${groupId}`} orgId={org.id} endpoint={selectedProfile.endpoint} members={members.data} />}
