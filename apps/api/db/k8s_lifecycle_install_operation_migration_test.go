...
func TestK8sLifecycleInstallOperationMigrationContract(t *testing.T) {
	up, err := os.ReadFile("migrations/0136_k8s_lifecycle_install_operations.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	down, err := os.ReadFile("migrations/0136_k8s_lifecycle_install_operations.down.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{
		...
		if !strings.Contains(upSQL, want) {
			t.Fatalf("0136 up missing %q", want)
		}
	}
	...
	if !(guardName < captureName) || !strings.Contains(upSQL, "CREATE TRIGGER "+guardName) {
		t.Fatal("0136 consumption guard must sort before the 0135 authorization-capture trigger")
	}
	downSQL := string(down)
	for _, want := range []string{
		...
		"restore a verified pre-0136 backup",
		...
		if !strings.Contains(downSQL, want) {
			t.Fatalf("0136 down missing %q", want)
		}
	}
}
