...
// Running them after 0135 proves the rolling bridge instead of merely testing a
...
func TestK8sLifecycleClaimRemintMigrationPostgres(t *testing.T) {
	...
	if adminURL == "" {
		t.Skip("set TUNNEX_TEST_DATABASE_URL for 0135 PostgreSQL mixed-version proof")
	}
	...
	if err := db.MigrateTo(dsn, 134); err != nil {
		t.Fatalf("migrate prerequisite chain through 0134: %v", err)
	}
	...
	t.Run("up follows enrollment token then node lock order", func(t *testing.T) {
		...
		if _, err := tx.Exec(ctx, `
			...
			t.Fatalf("seed 0134 join token: %v", err)
		}
		consumeNMinusOneLifecycleToken(t, ctx, tx, tokenHash)

		migrationDone := make(chan error, 1)
		go func() { migrationDone <- db.MigrateTo(dsn, 135) }()
		...
		for {
			...
			if queryErr != nil && !errors.Is(queryErr, pgx.ErrNoRows) {
				t.Fatalf("observe blocked 0135 up: %v", queryErr)
			}
			if time.Now().After(deadline) {
				t.Fatal("0135 up did not wait behind the in-flight token consumption")
			}
		...
		case migrationErr := <-migrationDone:
			if migrationErr != nil {
				t.Fatalf("0135 up after N-1 enrollment: %v", migrationErr)
			}
		case <-time.After(10 * time.Second):
			t.Fatal("0135 up did not finish after the N-1 enrollment committed")
		}
		...
		if err := pool.QueryRow(ctx, `SELECT lifecycle_claim FROM nodes WHERE cert_serial=$1`, certSerial).Scan(&nodeClaim); err != nil {
			t.Fatalf("read pre-0135 enrolled node: %v", err)
		}
		if nodeClaim != nil {
			t.Fatalf("pre-0135 legacy enrollment unexpectedly gained claim %s", *nodeClaim)
		}
	...
	if err := db.MigrateTo(dsn, 134); err != nil {
		t.Fatalf("empty 0135 down: %v", err)
	}
	if err := db.MigrateTo(dsn, 135); err != nil {
		t.Fatalf("0135 re-up: %v", err)
	}
	if version, dirty, ok, err := db.Version(dsn); err != nil || !ok || dirty || version != 135 {
		t.Fatalf("0135 version=%d dirty=%v ok=%v err=%v", version, dirty, ok, err)
	}
	...
	t.Run("usage sentinel survives lifecycle row deletion", func(t *testing.T) {
		...
		downSQL, err := os.ReadFile("migrations/0135_k8s_lifecycle_claim_remint.down.sql")
		if err != nil {
			t.Fatal(err)
		}
		...
		if _, downErr := downConn.Conn().PgConn().Exec(ctx, string(downSQL)).ReadAll(); downErr == nil || !strings.Contains(downErr.Error(), "database lifecycle is forward-only") {
			t.Fatalf("0135 down after lifecycle row deletion = %v", downErr)
		}
	...
	t.Run("down locks writers before checking forward-only state", func(t *testing.T) {
		...
		defer downConn.Release()
		const applicationName = "tnx_0135_down_lock_test"
		...
		for {
			...
			if time.Now().After(deadline) {
				t.Fatal("0135 down did not block behind the in-flight lifecycle writer")
			}
			time.Sleep(10 * time.Millisecond)
		}
		...
		case downErr := <-downDone:
			if downErr == nil || !strings.Contains(downErr.Error(), "database lifecycle is forward-only") {
				t.Fatalf("concurrent 0135 down error = %v", downErr)
			}
		case <-time.After(5 * time.Second):
			t.Fatal("0135 down did not recheck the guard after the writer committed")
		}
	...
	t.Run("populated lifecycle state refuses down migration", func(t *testing.T) {
		...
		err := db.MigrateTo(dsn, 134)
		if err == nil || !strings.Contains(err.Error(), "database lifecycle is forward-only") {
			t.Fatalf("populated 0135 down error = %v", err)
		}
		if version, dirty, ok, err := db.Version(dsn); err != nil || !ok || !dirty || version != 134 {
			t.Fatalf("refused 0135 down version=%d dirty=%v ok=%v err=%v; want 134/true/true", version, dirty, ok, err)
		}
