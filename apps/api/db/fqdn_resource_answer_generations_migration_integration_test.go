...
func TestFQDNResourceAnswerGenerationMigrationPostgres(t *testing.T) {
	...
	testURL := *base
	testURL.Path = "/" + name
	if err := db.MigrateTo(testURL.String(), 110); err != nil {
		t.Fatal(err)
	}
	...
	gen := uuid.New()
	exec(`INSERT INTO fqdn_resource_answer_generations(id,org_id,resource_id,generation,resolver_node_id,state,effective_ttl,resolved_at) VALUES($1,$2,$3,1,$4,'pending',interval '30 seconds',now())`, gen, org, resource, node)
	for i := 1; i <= 32; i++ {
	...
	second := uuid.New()
	exec(`INSERT INTO fqdn_resource_answer_generations(id,org_id,resource_id,generation,resolver_node_id,state,effective_ttl,resolved_at) VALUES($1,$2,$3,2,$4,'pending',interval '1 hour',now())`, second, org, resource, node)
	...
	if _, err := pool.Exec(ctx, `INSERT INTO fqdn_resource_answer_generations(id,org_id,resource_id,generation,resolver_node_id,state,effective_ttl,resolved_at) VALUES($1,$2,$3,1,$4,'pending',interval '30 seconds',now())`, uuid.New(), otherOrg, resource, node); err == nil {
