...
func TestK8sBaseAuthorityDeliveryKindMigrationContract(t *testing.T) {
	up, err := os.ReadFile("migrations/0130_k8s_base_authority_delivery_kind.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	down, err := os.ReadFile("migrations/0130_k8s_base_authority_delivery_kind.down.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{
		...
		"cannot roll back 0130: ordinary-base authority deliveries exist",
