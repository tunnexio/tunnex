...
BEGIN
    IF EXISTS (SELECT 1 FROM k8s_lifecycle_claim_usage)
       ...
       OR EXISTS (SELECT 1 FROM nodes WHERE lifecycle_claim IS NOT NULL) THEN
        RAISE EXCEPTION 'cannot roll back 0135 after Kubernetes lifecycle claim data exists: this database lifecycle is forward-only; restore a verified pre-0135 backup to run an N-1 control plane';
    END IF;
END $$;
