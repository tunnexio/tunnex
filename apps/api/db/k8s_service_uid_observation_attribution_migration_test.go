...
func TestK8sServiceUIDObservationAttributionMigrationContract(t *testing.T) {
	up, err := os.ReadFile("migrations/0121_k8s_service_uid_observation_attribution.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	if strings.Contains(upSQL, "UPDATE k8s_service_uid_observation_current SET") || strings.Contains(upSQL, "DELETE FROM k8s_service_uid_observation_current ") {
		t.Fatal("0121 up must preserve every existing current observation")
	}
	for _, want := range []string{
	...
	if strings.Contains(downSQL, "DELETE FROM k8s_service_uid_observation_current") {
		t.Fatal("0121 down must never delete observation data")
	}
}
...
func TestK8sServiceUIDObservationAttributionMigrationOrder(t *testing.T) {
	for _, direction := range []string{"up", "down"} {
		...
		if len(matches) != 1 || filepath.Base(matches[0]) != "0121_k8s_service_uid_observation_attribution."+direction+".sql" {
			t.Fatalf("0121 %s migration must be unique and ordered after 0120, found %v", direction, matches)
		}
		if _, err := os.Stat(filepath.Join("migrations", "0120_pool_vip_ownership_delivery_v3."+direction+".sql")); err != nil {
