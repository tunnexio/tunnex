...
type ListActivePolicyRulesForOrgRow struct {
	ID                uuid.UUID          `json:"id"`
	OrgID             uuid.UUID          `json:"org_id"`
	SrcGroupID        pgtype.UUID        `json:"src_group_id"`
	DstKind           string             `json:"dst_kind"`
...
func (q *Queries) ListActivePolicyRulesForOrg(ctx context.Context, orgID uuid.UUID) ([]ListActivePolicyRulesForOrgRow, error) {
	rows, err := q.db.Query(ctx, listActivePolicyRulesForOrg, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListActivePolicyRulesForOrgRow{}
	for rows.Next() {
		var i ListActivePolicyRulesForOrgRow
		if err := rows.Scan(
			&i.ID,
			&i.OrgID,
			&i.SrcGroupID,
...
const listApprovedK8sClusterScopeExpansions = `-- name: ListApprovedK8sClusterScopeExpansions :many
...
type ListApprovedK8sClusterScopeExpansionsRow struct {
...
func (q *Queries) ListApprovedK8sClusterScopeExpansions(ctx context.Context, orgID uuid.UUID) ([]ListApprovedK8sClusterScopeExpansionsRow, error) {
	rows, err := q.db.Query(ctx, listApprovedK8sClusterScopeExpansions, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListApprovedK8sClusterScopeExpansionsRow{}
	for rows.Next() {
		var i ListApprovedK8sClusterScopeExpansionsRow
		if err := rows.Scan(&i.PolicyRuleID, &i.ServiceChildID); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
