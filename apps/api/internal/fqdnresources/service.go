...
func (s *Service) Create(ctx context.Context, org uuid.UUID, in Input) (Resource, error) {
	...
	err := s.pool.QueryRow(ctx, `INSERT INTO fqdn_resources (org_id,name,fqdn,protocol,port_low,port_high,label,resolver_site_id,resolver_node_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id`, org, in.Name, in.FQDN, in.Protocol, in.PortLow, in.PortHigh, in.Label, site, gateway).Scan(&id)
	if err != nil {
		return Resource{}, writeErr(err)
	}
	return s.Get(ctx, org, id)
}
func (s *Service) Update(ctx context.Context, org, id uuid.UUID, in Input) (Resource, error) {
	if err := valid(&in); err != nil {
		return Resource{}, err
	}
	var site, gateway *uuid.UUID
	...
	ct, err := s.pool.Exec(ctx, `UPDATE fqdn_resources SET name=$3,fqdn=$4,protocol=$5,port_low=$6,port_high=$7,label=$8,resolver_site_id=$9,resolver_node_id=$10 WHERE id=$1 AND org_id=$2`, id, org, in.Name, in.FQDN, in.Protocol, in.PortLow, in.PortHigh, in.Label, site, gateway)
	if err != nil {
		return Resource{}, writeErr(err)
	}
...
func (s *Service) Impact(ctx context.Context, org, id uuid.UUID) (Impact, error) {
	var i Impact
	err := s.pool.QueryRow(ctx, `SELECT (SELECT count(*) FROM fqdn_resource_rule_references WHERE org_id=$1 AND resource_id=$2), EXISTS(SELECT 1 FROM fqdn_resource_answer_generations WHERE org_id=$1 AND resource_id=$2 AND state='active') FROM fqdn_resources r WHERE r.org_id=$1 AND r.id=$2`, org, id).Scan(&i.ReferencingRuleCount, &i.GenerationWithdrawalRequired)
	if err == pgx.ErrNoRows {
...
func (s *Service) Delete(ctx context.Context, org, id uuid.UUID) error {
	i, err := s.Impact(ctx, org, id)
	if err != nil {
		return err
	}
	if i.ReferencingRuleCount > 0 || i.GenerationWithdrawalRequired {
	...
	var historical bool
	if err := s.pool.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM fqdn_resource_answer_generations WHERE org_id=$1 AND resource_id=$2)`, org, id).Scan(&historical); err != nil {
		return err
	}
	if historical {
	...
	ct, err := s.pool.Exec(ctx, `DELETE FROM fqdn_resources WHERE org_id=$1 AND id=$2`, org, id)
	if err != nil {
		return writeErr(err)
	}
	if ct.RowsAffected() == 0 {
...
func (s *Service) SetSetting(ctx context.Context, org uuid.UUID, enabled bool) error {
	ct, err := s.pool.Exec(ctx, `UPDATE organizations SET fqdn_resources_enabled=$2,updated_at=now() WHERE id=$1 AND deleted_at IS NULL`, org, enabled)
	if err != nil {
		return err
	}
	if ct.RowsAffected() == 0 {
