...
func issueKubernetesOwnershipBaseAuthorityTx(ctx context.Context, tx pgx.Tx, issue KubernetesOwnershipBaseAuthorityIssue) (KubernetesOwnershipBaseAuthorityIssueResult, error) {
	...
	var err error
	if issue.OrdinaryBaseUpdate {
		pendingTransition, pendingErr := latestKubernetesOwnershipAuthorityIsUnacknowledgedTransition(ctx, tx, orgID, siteID, nodeID)
		if pendingErr != nil {
			return KubernetesOwnershipBaseAuthorityIssueResult{}, pendingErr
		}
