...
func TestPostgresHandoffSchedulerMigrationGateRequires0129(t *testing.T) {
	...
	for _, version := range []int{85, 118, 119, 120, 122, 128} {
		...
		if (&PostgresHandoffSchedulerMigrationGate{pool: pool}).ready(ctx) {
			pool.Close()
			t.Fatalf("schema %04d must not enable the scheduler before 0129 delivery-kind authority", version)
		}
		pool.Close()
	}
	...
	defer pool.Close()
	if !(&PostgresHandoffSchedulerMigrationGate{pool: pool}).ready(ctx) {
		t.Fatal("clean schema 0129 must satisfy the scheduler migration gate")
	}
}
