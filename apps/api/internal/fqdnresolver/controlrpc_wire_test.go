...
func TestGatewayDNSRPCV1WireShape(t *testing.T) {
	...
	wantKeys := []string{"version", "request_id", "org_id", "resource_id", "site_id", "gateway_id", "resolver_config_id", "resolver_config_version", "resolver_endpoints", "hostname", "record_types", "deadline"}
	...
	wantKeys = []string{"version", "request_id", "org_id", "resource_id", "site_id", "gateway_id", "resolver_config_id", "resolver_config_version", "hostname", "record_types", "observed_at", "status", "records"}
	if len(got) != len(wantKeys) {
