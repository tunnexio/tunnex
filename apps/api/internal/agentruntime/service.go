...
func (s *Service) Poll(ctx context.Context, id Identity, appliedRevision int64, clientVersion string) (Config, bool, error) {
	if appliedRevision < 0 || strings.TrimSpace(clientVersion) == "" || len(clientVersion) > 128 {
...
func (s *Service) PollWait(ctx context.Context, id Identity, appliedRevision int64, clientVersion string, wait time.Duration) (Config, bool, error) {
	if wait <= 0 {
		return s.Poll(ctx, id, appliedRevision, clientVersion)
	}
	tick := s.pollTick
	if tick <= 0 {
		tick = time.Second
