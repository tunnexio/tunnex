...
func (a *accessLogAdapter) List(ctx context.Context, orgID uuid.UUID, identity *accessEventIdentityFilter, deniesOnly bool, cursorTS time.Time, cursorID uuid.UUID, limit int32) ([]accesslog.Event, error) {
	...
	if identity == nil {
		...
		argID := pgtype.UUID{Bytes: identity.ID, Valid: true}
		switch identity.Kind {
		case accessEventIdentityAgent:
			if deniesOnly {
		...
		case accessEventIdentityDevice:
			if deniesOnly {
				rows, err = a.q.ListAccessDeniesByDevice(ctx, sqlc.ListAccessDeniesByDeviceParams{OrgID: orgID, SrcDeviceID: argID, BeforeCreatedAt: cursorTS, BeforeID: cursorID, PageLimit: limit})
			} else {
				rows, err = a.q.ListAccessEventsByDevice(ctx, sqlc.ListAccessEventsByDeviceParams{OrgID: orgID, SrcDeviceID: argID, BeforeCreatedAt: cursorTS, BeforeID: cursorID, PageLimit: limit})
			}
		case accessEventIdentityUser:
			if deniesOnly {
				rows, err = a.q.ListAccessDeniesByUser(ctx, sqlc.ListAccessDeniesByUserParams{OrgID: orgID, SrcUserID: argID, BeforeCreatedAt: cursorTS, BeforeID: cursorID, PageLimit: limit})
			} else {
				rows, err = a.q.ListAccessEventsByUser(ctx, sqlc.ListAccessEventsByUserParams{OrgID: orgID, SrcUserID: argID, BeforeCreatedAt: cursorTS, BeforeID: cursorID, PageLimit: limit})
			}
		default:
			return nil, fmt.Errorf("unknown access-event identity filter kind %d", identity.Kind)
		}
	}
	if err != nil {
		return nil, err
