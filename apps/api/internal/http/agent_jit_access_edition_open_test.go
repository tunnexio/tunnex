...
func TestAgentJITAccessPermissionBeforeEdition(t *testing.T) {
	...
	if _, err := s.SetOrganizationAgentJITAccessEnabled(with(member, rbac.RoleMember), setting); !hasCode(err, 403, "forbidden") {
		t.Fatalf("member must fail permission before edition: %v", err)
	}
	if _, err := s.SetOrganizationAgentJITAccessEnabled(with(owner, rbac.RoleOwner), setting); !hasCode(err, 403, "edition_required") {
		t.Fatalf("owner must reach edition gate: %v", err)
	}
	...
	if _, err := s.CreateAgentAccessRequest(with(owner, rbac.RoleOwner), api.CreateAgentAccessRequestRequestObject{OrgId: org, Body: body}); !hasCode(err, 403, "edition_required") {
		t.Fatalf("authorized create must reach edition gate: %v", err)
	}
}
