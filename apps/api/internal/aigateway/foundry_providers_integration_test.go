...
func TestAIFoundryProvidersPostgres(t *testing.T) {
	...
	os.WriteFile(path, []byte(`{"endpoints":[{"name":"Foundry","provider":"azure_foundry","url":"https://fixture.services.ai.azure.com/openai","allowed_cidrs":["10.20.0.0/16"]},{"name":"Custom","url":"http://custom.example:8080","allowed_cidrs":["10.20.0.0/16"]}],"protected_hosts":["control.invalid"]}`), 0600)
	policy, err := aiegress.LoadPolicy(path)
	if err != nil {
		t.Fatal(err)
	}
	f.policies.ConfigureCustomProviders(policy)
	if !f.policies.FoundryAvailable() || !f.policies.CustomAvailable() || len(f.policies.ApprovedFoundryEndpoints()) != 1 || len(f.policies.ApprovedCustomEndpoints()) != 1 {
		t.Fatal("inventory separation")
	}
	endpoint, secret := "https://fixture.services.ai.azure.com/openai", "fixture-client-key"
	in := ProviderInput{Provider: "azure_foundry", Name: "Foundry endpoint", EndpointURL: &endpoint, Secret: &secret, Models: []string{"my-deployment"}, Enabled: true}
	p, err := f.policies.CreateProvider(ctx, f.org, f.owner, in)
	if err != nil || p.Status != "applied" {
		t.Fatal(p, err)
	}
	if p.Models[0] != "custom-"+p.ID.String()+"/my-deployment" || engine.values[p.KeyID].Provider != "custom-"+p.ID.String() {
