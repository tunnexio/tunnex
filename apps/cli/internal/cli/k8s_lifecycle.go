...
type installState struct {
	...
	pvcLifecycleGeneration                int
	pvcLifecycleRequestID                 string
	pvcLifecycleConsumedAt                time.Time
	pvcBoundNodeID                        string
}
...
func (s installState) fingerprint() string {
	encoded := strings.Join([]string{
		...
		strconv.Itoa(s.pvcLifecycleGeneration), s.pvcLifecycleRequestID,
		s.pvcLifecycleConsumedAt.UTC().Format(time.RFC3339Nano), s.pvcBoundNodeID,
	}, "\x00")
...
func discoverInstallState(ctx context.Context, runner k8sRunner, o installOptions) (installState, error) {
	...
	if o.mode == "reuse" {
		...
		if err := validateRetainedPVCMetadata(pvc, o.existingClaim, o.release, o.namespace); err != nil {
			return installState{}, err
		}
		organizationID, lifecycleClaim, present, provenanceErr := exactLifecycleProvenance(pvc)
		if provenanceErr != nil {
			return installState{}, fmt.Errorf("retained claim %q lifecycle provenance is invalid: %w", o.existingClaim, provenanceErr)
		}
		if !present {
			return installState{}, fmt.Errorf("retained claim %q has no lifecycle provenance; lifecycle CLI reuse cannot claim zero-touch recovery for a pre-contract/manual Helm PVC", o.existingClaim)
		}
	...
	if pvc != nil {
		...
		if !state.retrySecret && !state.anchorExists && len(releases) == 0 {
			return installState{}, fmt.Errorf("state claim %q exists without retry Secret %q; refusing a fresh enrollment over orphan state. If it contains an enrolled identity with canonical lifecycle provenance, use --mode reuse --existing-claim %s; otherwise follow the documented direct/manual Helm tokenless reuse path", expectedClaim, secretName, expectedClaim)
		}
		if state.anchorExists {
			organizationID, lifecycleClaim, present, provenanceErr := exactLifecycleProvenance(*pvc)
			if provenanceErr != nil {
				return installState{}, fmt.Errorf("retained enroll claim %q lifecycle provenance is invalid: %w", expectedClaim, provenanceErr)
			}
			if !present || organizationID != anchor.orgID || lifecycleClaim != anchor.lifecycleClaim {
				return installState{}, fmt.Errorf("retained enroll claim %q lifecycle provenance does not match its owned lifecycle anchor; refusing to relabel retained identity state", expectedClaim)
			}
			state.pvcOrganizationID = organizationID
			state.pvcLifecycleClaim = lifecycleClaim
		}
...
func verifyInstalledGatewayState(ctx context.Context, runner k8sRunner, prepared preparedInstall) error {
	...
	} else if o.mode == "reuse" {
		if err := validateExactPVCLifecycleProvenance(pvc, prepared.state.pvcOrganizationID, prepared.state.pvcLifecycleClaim); err != nil {
			return err
		}
		state := prepared.state
		if err := validateRetainedPVCReuseControlPlane(ctx, prepared.cp, prepared.org, o, &state); err != nil {
			return fmt.Errorf("re-prove retained claim control-plane identity after Helm: %w", err)
		}
	}
	return nil
}
...
func exactLifecycleProvenance(pvc pvcView) (organizationID, lifecycleClaim string, present bool, err error) {
	...
	if organizationPresent != claimPresent || strings.TrimSpace(organizationID) != organizationID || strings.TrimSpace(lifecycleClaim) != lifecycleClaim || organizationID == "" || lifecycleClaim == "" {
		return "", "", false, errors.New("state claim has partial or malformed lifecycle provenance; refusing lifecycle mutation")
	}
	...
	if orgErr != nil || claimErr != nil || parsedOrg == uuid.Nil || parsedClaim == uuid.Nil || parsedOrg.String() != organizationID || parsedClaim.String() != lifecycleClaim {
		return "", "", false, errors.New("state claim lifecycle provenance is not two canonical UUIDs; refusing lifecycle mutation")
	}
...
func validatePVCLifecycleProvenance(pvc pvcView, anchor lifecycleAnchorMetadata) error {
	return validateExactPVCLifecycleProvenance(pvc, anchor.orgID, anchor.lifecycleClaim)
}

func validateExactPVCLifecycleProvenance(pvc pvcView, expectedOrganizationID, expectedLifecycleClaim string) error {
	...
	if !present || organizationID != expectedOrganizationID || lifecycleClaim != expectedLifecycleClaim {
		return fmt.Errorf("gateway state claim lifecycle provenance does not match the approved organization %s and claim %s", expectedOrganizationID, expectedLifecycleClaim)
	}
	return nil
}

// validateRetainedPVCReuseControlPlane turns PVC provenance into reuse
// authority only after the selected organization proves that the exact opaque
// claim was consumed into one canonical node UUID. Mutable node names are not
// compared and can never substitute for the claim-to-node binding.
func validateRetainedPVCReuseControlPlane(
	ctx context.Context,
	cp k8sControlPlane,
	organization k8sOrganization,
	o installOptions,
	state *installState,
) error {
	if o.mode != "reuse" {
	...
	if state == nil || !state.pvcExists || state.pvcName != o.existingClaim {
		return errors.New("reuse requires one exact inspected retained PVC identity")
	}
	if !canonicalNonNilUUID(organization.id) || state.pvcOrganizationID != organization.id {
		return fmt.Errorf("retained claim %q is pinned to a different organization than the selected current membership", o.existingClaim)
	}
	if !canonicalNonNilUUID(state.pvcLifecycleClaim) {
		return fmt.Errorf("retained claim %q lacks one canonical lifecycle claim", o.existingClaim)
	}
	status, err := cp.GetLifecycleClaimStatus(ctx, organization.id, state.pvcLifecycleClaim)
	if errors.Is(err, errK8sLifecycleClaimNotFound) {
		return fmt.Errorf("retained claim %q lifecycle claim is absent from the selected organization", o.existingClaim)
	}
	if err != nil {
		return fmt.Errorf("read retained claim %q lifecycle provenance: %w", o.existingClaim, err)
	}
	if status.claim != state.pvcLifecycleClaim || !canonicalNonNilUUID(status.claim) {
		return fmt.Errorf("retained claim %q control-plane lifecycle claim does not match its immutable PVC provenance", o.existingClaim)
	}
	if status.state != "consumed" || status.generation <= 0 || !canonicalNonNilUUID(status.requestID) ||
		status.consumedAt == nil || status.consumedAt.IsZero() || status.abortedAt != nil || !canonicalNonNilUUID(status.nodeID) {
		return fmt.Errorf("retained claim %q control-plane lifecycle claim is not one exact consumed canonical node identity", o.existingClaim)
	}
	approvedIdentityPresent := state.pvcLifecycleGeneration != 0 || state.pvcLifecycleRequestID != "" ||
		!state.pvcLifecycleConsumedAt.IsZero() || state.pvcBoundNodeID != ""
	if approvedIdentityPresent {
		if state.pvcLifecycleGeneration <= 0 || !canonicalNonNilUUID(state.pvcLifecycleRequestID) ||
			state.pvcLifecycleConsumedAt.IsZero() || !canonicalNonNilUUID(state.pvcBoundNodeID) {
			return fmt.Errorf("retained claim %q approved control-plane lifecycle identity is incomplete", o.existingClaim)
		}
		if state.pvcLifecycleGeneration != status.generation || state.pvcLifecycleRequestID != status.requestID ||
			!state.pvcLifecycleConsumedAt.Equal(*status.consumedAt) || state.pvcBoundNodeID != status.nodeID {
			return fmt.Errorf("retained claim %q control-plane lifecycle identity changed after approval", o.existingClaim)
		}
	}
	state.pvcLifecycleGeneration = status.generation
	state.pvcLifecycleRequestID = status.requestID
	state.pvcLifecycleConsumedAt = *status.consumedAt
	state.pvcBoundNodeID = status.nodeID
	return nil
}
