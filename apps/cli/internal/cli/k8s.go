...
Install flags:
  ...
  --mode enroll|reuse         reuse requires --existing-claim and mints no token
...
func prepareInstall(ctx context.Context, o installOptions, deps k8sDeps) (prepared preparedInstall, retErr error) {
	...
	storageState := "existing retained claim"
	operations := []string{hostPostureOperation, "recheck release and claim ownership", "create namespace only if absent", "Helm atomic install", "verify Deployment readiness", "verify Service endpoint"}
	if o.mode == "enroll" {
		secretName = o.release + "-bootstrap"
...
func runK8sInstall(ctx context.Context, args []string, deps k8sDeps) (retErr error) {
	...
	if o.mode == "reuse" {
		...
		reuseFence, err = acquireRetainedStateFence(ctx, deps, binding, retainedStateFenceOperationReuse, func(reproofCtx context.Context) error {
			return reproveReuseStateFence(reproofCtx, deps, o, binding, prepared.state.fingerprint())
		})
		if err != nil {
			return err
		}
		...
		defer func() {
			...
			if !safeToRelease {
				if cause := context.Cause(reuseRenewal.ctx); cause != nil {
					...
					safeProofErr = reproveReuseStateFence(reuseRenewal.ctx, deps, o, binding, "")
	...
	if currentState.fingerprint() != prepared.state.fingerprint() {
		return errors.New("Kubernetes release/Secret/claim state changed after plan approval; no token was minted and nothing was installed — rerun the command to review a fresh plan")
	}
