...
func reproveReuseStateFence(ctx context.Context, deps k8sDeps, prepared preparedInstall, binding retainedStateFenceBinding, approvedFingerprint string) error {
	o := prepared.options
	...
	if err := validateRetainedPVCReuseControlPlane(ctx, prepared.cp, prepared.org, o, &state); err != nil {
		return fmt.Errorf("re-prove retained claim control-plane identity for Lease resume/takeover: %w", err)
	}
	if approvedFingerprint != "" && state.fingerprint() != approvedFingerprint {
		return errors.New("Kubernetes or control-plane release/claim identity differs from the approved reuse plan")
	}
