...
func (m *Manager) ReconcileOnce(ctx context.Context) error {
	...
	// Once teardown intent is durable, finish that exact old epoch before
	// considering a newly observed owner. This makes every already-absent exact
	// artifact a cleanup replay result without turning active+missing-link into
	// an implicit reboot rebootstrap path.
	if haveJournal && journal.State == StateRestoring {
		return m.finishRestore(ctx, &journal, now)
	}
	if len(owners) > 0 {
	...
	journal.LastError = ""
	journal.UpdatedAt = now
	// This durable state is the teardown admission boundary. No kernel cleanup
	// may run while the crash authority still describes an active epoch.
	if err := m.store.SaveJournal(journal); err != nil {
		return m.block(now, nil, fmt.Errorf("persist host-posture teardown intent: %w", err))
	}
	return m.finishRestore(ctx, &journal, now)
}

func (m *Manager) finishRestore(ctx context.Context, journal *Journal, now time.Time) error {
